...
class Cart extends CartCore {
    ...
    */
    public function getPackageShippingCost(
        ...
        if (!isset($cache[$key])) {
            // 1) Ejecutamos el hook para obtener fees adicionales
            $total = 0;
            $return = false;
            ...
            if ($return) {
                // Si el hook devolvió directamente, usamos ese total
                $cache[$key] = ($total !== false ? (float) Tools::ps_round((float) $total, 2) : false);
            } else {
                // 2) Obtenemos coste base respetando el flag $use_tax
                $baseCost = parent::getPackageShippingCost(
                    $id_carrier,
                    $use_tax, // RESPETAMOS el flag original
                    $default_country,
                    $product_list,
                    $id_zone,
                    $keepOrderPrices
                ...
                // 3) Definimos fees NETOS base
                $netFees = (float)$total; // Fees del hook o 0 si no hay hook

                // 4) Si piden bruto, calculamos IVA sobre los fees
                if ($use_tax && $netFees > 0) {
                    // Obtenemos configuración de impuestos
                    ...
                    $countryId = $default_country ? (int)$default_country->id : (int)$this->getDeliveryCountry()->id;
                    ...
                    // Aplicamos IVA: bruto = neto * (1 + tasa/100)
                    $fees = Tools::ps_round($netFees * (1 + $rate/100), 2);
                } else {
                    // Si piden neto o no hay fees, usamos el importe sin IVA
                    $fees = $netFees;
                }

                // 5) Sumamos base + fees y redondeamos
