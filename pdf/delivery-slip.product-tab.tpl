...
<table class="product" width="100%" cellpadding="4" cellspacing="0">
	...
	<thead>
		<tr>
			...
			<th class="product header small" width="40%">{l s='Product' d='Shop.Pdf' pdf='true'}</th>
			<th class="product header small" width="10%">{l s='Qty' d='Shop.Pdf' pdf='true'}</th>
			<th class="product header small" width="15%">{l s='weight' d='Shop.Pdf' pdf='true'}</th>

			<th class="product header small" width="15%">{l s='total weight' d='Shop.Pdf' pdf='true'}</th>
		</tr>
	</thead>
	...
	<tbody>
		...
		{foreach $order_details as $order_detail}
			...
			<tr class="product {$bgcolor_class}">
				...
				<td class="product center">
					{$order_detail.weight|string_format:"%.2f"} Kgs.
				</td>
				<td class="product center">
					{assign var=concept_weight value=$order_detail.weight * $order_detail.product_quantity}
					{assign var=total_weight value=$total_weight + $concept_weight}
					{$concept_weight|string_format:"%.2f"} Kgs.
				</td>
		...
		{/foreach}
			...
			<tr class="product {$bgcolor_class}">
				<td class="product center" colspan="4"></td>
				<td class="product center"><span style="font-size:10px; font-weight:bold">{$total_weight|string_format:"%.2f"} Kgs.</span></td>
			</tr>
		<!-- END PRODUCTS -->
	</tbody>
