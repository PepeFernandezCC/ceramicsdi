...
<table id="summary-tab" width="100%">
	<tr>
		...
		<th class="header small" valign="middle">Factura a la que rectifica</th>
		<th class="header small" valign="middle">Fecha de la factura a la que rectifica</th>
		{if $addresses.invoice->vat_number}
			<th class="header small" valign="middle">{l s='VAT Number' d='Shop.Pdf' pdf='true'}</th>
		{/if}
	</tr>
	<tr>
		<td class="center small white">{$order->getUniqIdReference()}</td>
		<td class="center small white">{dateFormat date=$order->date_add full=0}</td>
		<td class="center small white">{$invoice_complete_number}</td>
