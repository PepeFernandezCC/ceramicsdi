...
if (!$revisions = LsRevisions::snapshots($id)) {
    return;
}
...
foreach ($revisions as $key => $revision) {
    ...
    $revisions[$key]->time_diff = sprintf(ls__(' %s ago'), ls_human_time_diff($revision->date_c));
...
<div id="ls-revisions">
    ...
    <div class="wrap">
        <h2>
            ...
            <a href="<?php echo ls_admin_url('?controller=AdminLayerSlider&action=edit&id=' . $id); ?>" class="add-new-h2"><?php ls_e('Back to Slider'); ?></a>
        </h2>
            ...
            <div id="ls-revisions-selected">
                <table>
                    <tr>
                        ...
                        <td>
                            <?php printf(ls__('Selected Revision by %s'), '<strong class="author">' . $revision->nickname . '</strong>'); ?>
                ...
                <button class="button button-primary button-hero"><?php ls_e('Revert to This Revision'); ?></button>
            </div>
        ...
        <div class="ls-notification-info">
            ...
            <?php ls_e('Reverting a slider to an earlier version adds another snapshot to Revisions, which can also be reverted if you change your mind and would rather return to the original copy.'); ?>
            <?php ls_e('Slider Revisions also saves the undo/redo controls. Even if there is no perfect snapshot, you will be able to undo the changes in-between to find what you are looking for.'); ?>
        </div>

        <h2 class="ls-revisions-h2"><?php ls_e('Preview for Selected Revision'); ?></h2>
        <div id="ls-slider-form">
            <div id="ls-layer-tabs">
                <?php
                foreach ($slider['layers'] as $key => $layer) {
                    $active = empty($key) ? 'active' : '';
                    $name = !empty($layer['properties']['title']) ? $layer['properties']['title'] : sprintf(ls__('Slide #%d'), $key + 1);
                            ...
                            <tr id="slider-editor-toolbar">
                                <td >
                                    ...
                                    <div class="ls-editor-preview">
                                        <button type="button" class="button ls-preview-button"><?php ls_e('Preview Slide'); ?></button>
                                    </div>
