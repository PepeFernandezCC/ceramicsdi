...
// $postTaxonomies = ls_get_taxonomies(['_builtin' => false], 'objects');
$postTaxonomies = [];
?>
<div id="ls-screen-options" class="metabox-prefs hidden">
    <div id="screen-options-wrap" class="hidden">
        <form id="ls-screen-options-form" method="post">
            <h5><?php ls_e('Use features'); ?></h5>
            <label>
                <input type="checkbox" name="showTooltips"<?php echo 'true' == $lsScreenOptions['showTooltips'] ? ' checked="checked"' : ''; ?>> <?php ls_e('Tooltips'); ?>
            </label>
            <label>
                <input type="checkbox" name="useKeyboardShortcuts"<?php echo 'true' == $lsScreenOptions['useKeyboardShortcuts'] ? ' checked="checked"' : ''; ?>> <?php ls_e('Keyboard shortcuts'); ?>
            </label>
            <label>
                <input type="checkbox" name="useNotifyOSD"<?php echo 'true' == $lsScreenOptions['useNotifyOSD'] ? ' checked="checked"' : ''; ?>> <?php ls_e('On Screen Notifications'); ?>
            </label>
        </form>
    </div>
...
if (empty($slider['properties']['new']) && empty($slider['properties']['type'])) {
    if (!empty($slider['properties']['forceresponsive'])) {
        ...
        if (false !== strpos($slider['properties']['width'], '%')) {
...
foreach ($slider['layers'] as $slideKey => $slideVal) {
    ...
    if (!empty($slideVal['sublayers']) && is_array($slideVal['sublayers'])) {
        ...
        foreach ($slideVal['sublayers'] as $layerKey => $layerVal) {
            ...
            $layerVal['styles'] = !empty($layerVal['styles']) ? (object) json_decode(stripslashes($layerVal['styles'])) : new stdClass();
            $layerVal['transition'] = !empty($layerVal['transition']) ? (object) json_decode(stripslashes($layerVal['transition'])) : new stdClass();
            ...
            if (!empty($layerVal['transition']->showuntil)) {
                $layerVal['transition']->startatout = 'transitioninend + ' . $layerVal['transition']->showuntil; // @phpstan-ignore-line
                $layerVal['transition']->startatouttiming = 'transitioninend'; // @phpstan-ignore-line
                $layerVal['transition']->startatoutvalue = $layerVal['transition']->showuntil; // @phpstan-ignore-line
                unset($layerVal['transition']->showuntil);
            }

            if (!empty($layerVal['transition']->parallaxlevel)) {
                $layerVal['transition']->parallax = true; // @phpstan-ignore-line
            }
...
<form method="post" id="ls-slider-form" novalidate="novalidate" autocomplete="off">
    ...
    <div class="wrap">
        ...
        <h2>
            ...
            <a href="<?php echo ls_admin_url('?controller=AdminLayerSlider'); ?>" class="add-new-h2"><?php ls_e('Back to the list'); ?></a>
        </h2>
    ...
    <div id="ls-pages">
        ...
        <div class="ls-page <?php echo $slidesTabClass; ?>">
            ...
            foreach ($slider['layers'] as $key => $layer) {
                ...
                $image = ls_apply_filters('ls_get_image', $bgImageId, $bgImage); ?>
            ...
            <?php
                ...
                <a href="#"  title="<?php ls_e('Add new slide'); ?>" class="unsortable" id="ls-add-layer"><i class="dashicons dashicons-plus"></i></a>
        ...
        <div class="ls-page ls-callback-page">
            ...
            <div class="ls-notification-info">
                ...
                <?php printf(ls__('Please read our %sonline documentation%s before start using the API. Creative Slider 6 introduced an entirely new API model with different events and methods.'), '<a href="http://docs.webshopworks.com/creative-slider/55-advanced-customization/226-creative-slider-api" target="_blank">', '</a>'); ?>
            </div>
            ...
            <div class="ls-box ls-callback-box">
                <h3 class="header">
                    sliderWillLoad
                    <figure><span>|</span> <?php ls_e('Fires before parsing user settings and rendering the UI.'); ?></figure>
                </h3>
                <div>
            ...
            <div class="ls-box ls-callback-box">
                <h3 class="header">
                    sliderDidLoad
                    <figure><span>|</span> <?php ls_e('Fires when the slider is fully initialized and its DOM nodes become accessible.'); ?></figure>
                </h3>
                <div>
            ...
            <div class="ls-callback-separator"><?php ls_e('Resize Events'); ?></div>


            <div class="ls-box ls-callback-box side">
                <h3 class="header">
                    sliderWillResize
                    <figure><span>|</span> <?php ls_e('Fires before the slider renders resize events.'); ?></figure>
                </h3>
                <div>
            ...
            <div class="ls-box ls-callback-box">
                <h3 class="header">
                    sliderDidResize
                    <figure><span>|</span> <?php ls_e('Fires after the slider has rendered resize events.'); ?></figure>
                </h3>
                <div>
            ...
            <div class="ls-callback-separator"><?php ls_e('Slideshow Events'); ?></div>


            <div class="ls-box ls-callback-box">
                <h3 class="header">
                    slideshowStateDidChange
                    <figure><span>|</span> <?php ls_e('Fires upon every slideshow state change, which may not influence the playing status.'); ?></figure>
                </h3>
                <div>
            ...
            <div class="ls-box ls-callback-box">
                <h3 class="header">
                    slideshowDidPause
                    <figure><span>|</span> <?php ls_e('Fires when the slideshow pauses from playing status.'); ?></figure>
                </h3>
                <div>
            ...
            <div class="ls-box ls-callback-box side">
                <h3 class="header">
                    slideshowDidResume
                    <figure><span>|</span> <?php ls_e('Fires when the slideshow resumes from paused status.'); ?></figure>
                </h3>
                <div>
            ...
            <div class="ls-callback-separator"><?php ls_e('Slide Change Events'); ?></div>


            <div class="ls-box ls-callback-box">
                <h3 class="header">
                    slideChangeWillStart
                    <figure><span>|</span> <?php ls_e('Signals when the slider wants to change slides, and is your last chance to divert it or intervene in any way.'); ?></figure>
                </h3>
                <div>
            ...
            <div class="ls-box ls-callback-box">
                <h3 class="header">
                    slideChangeDidStart
                    <figure><span>|</span> <?php ls_e('Fires when the slider has started a slide change.'); ?></figure>
                </h3>
                <div>
            ...
            <div class="ls-box ls-callback-box">
                <h3 class="header">
                    ...
                    <figure><span>|</span> <?php ls_e('Fires before completing a slide change.'); ?></figure>
                </h3>
                <div>
            ...
            <div class="ls-box ls-callback-box">
                <h3 class="header">
                    ...
                    <figure><span>|</span> <?php ls_e('Fires after a slide change has completed and the slide indexes have been updated. '); ?></figure>
                </h3>
                <div>
            ...
            <div class="ls-callback-separator"><?php ls_e('Slide Timeline Events'); ?></div>

            <div class="ls-box ls-callback-box">
                <h3 class="header">
                    slideTimelineDidCreate
                    <figure><span>|</span> <?php ls_e("Fires when the current slide's animation timeline (e.g. your layers) becomes accessible for interfacing."); ?></figure>
                </h3>
                <div>
            ...
            <div class="ls-box ls-callback-box">
                <h3 class="header">
                    ...
                    <figure><span>|</span> <?php ls_e('Fires rapidly (at each frame) throughout the entire slide while playing, including reverse playback.'); ?></figure>
                </h3>
                <div>
            ...
            <div class="ls-box ls-callback-box">
                <h3 class="header">
                    ...
                    <figure><span>|</span> <?php ls_e("Fires when the current slide's animation timeline (e.g. your layers) has started playing."); ?></figure>
                </h3>
                <div>
            ...
            <div class="ls-box ls-callback-box">
                <h3 class="header">
                    ...
                    <figure><span>|</span> <?php ls_e("Fires when the current slide's animation timeline (e.g. layer transitions) has completed."); ?></figure>
                </h3>
                <div>
            ...
            <div class="ls-box ls-callback-box">
                <h3 class="header">
                    ...
                    <figure><span>|</span> <?php ls_e('Fires when all reversed animations have reached the beginning of the current slide.'); ?></figure>
                </h3>
                <div>
            ...
            <div class="ls-callback-separator"><?php ls_e('Destroy Events'); ?></div>


            <div class="ls-box ls-callback-box">
                <h3 class="header">
                    sliderDidDestroy
                    <figure><span>|</span> <?php ls_e('Fires when the slider destructor has finished and it is safe to remove the slider from the DOM.'); ?></figure>
                </h3>
                <div>
            ...
            <div class="ls-box ls-callback-box">
                <h3 class="header">
                    sliderDidRemove
                    <figure><span>|</span> <?php ls_e('Fires when the slider has been removed from the DOM when using the <i>destroy</i> API method.'); ?></figure>
                </h3>
                <div>
    ...
    <div class="ls-publish">
        <button type="submit" class="button button-primary button-hero"><?php ls_e('Save changes'); ?></button>
        <div class="ls-save-shortcode">
        <?php $revisions = LsRevisions::count($id); ?>
        <?php if ($revisions > 1) { ?>
            <p class="revisions"><span>
                <i class="dashicons dashicons-backup"></i><?php printf(ls__('Revisions Available:'), $revisions); ?></span><br>
                <a href="<?php echo ls_admin_url('?controller=AdminLayerSliderRevisions&id=' . $id); ?>"><?php printf(ls__('Browse %d Revisions'), $revisions); ?></a>
            </p>
        <?php } ?>
            <p><span><?php ls_e('Use shortcode:'); ?></span><br><span>[creativeslider id="<?php echo !empty($slider['properties']['slug']) ? $slider['properties']['slug'] : $id; ?>"]</span></p>
            <p><span><?php ls_e('Use PHP function:'); ?></span><br><span>&lt;?php creativeslider(<?php echo !empty($slider['properties']['slug']) ? "'{$slider['properties']['slug']}'" : $id; ?>) ?&gt;</span></p>
        </div>
    </div>
</form>
