...
function layerslider_enqueue_content_res()
{
    ...
    ls_register_script('layerslider', LS_VIEWS_URL . 'js/layerslider/layerslider.webshopworks.jquery.js', ['jquery'], LS_PLUGIN_VERSION, $footer);
    ...
    ls_register_script('layerslider-origami', LS_VIEWS_URL . 'js/layerslider/plugins/origami/layerslider.origami.js', ['jquery'], LS_PLUGIN_VERSION, $footer);
    ...
    ls_register_style('ce-font-awesome', LS_VIEWS_URL . 'lib/font-awesome/css/font-awesome.min.css', false, '4.7.0');
    ...
    Media::addJsDef(['LS_Meta' => $LS_Meta]);
    ...
    if (ls_get_option('ls_load_fontawesome', true)) {
        ls_enqueue_style('ce-font-awesome');
    }
}
...
function layerslider_enqueue_admin_res()
{
    ...
    if (false !== strpos($screen->base, 'layerslider')) {
        // New Media Library
        // if (function_exists('ls_enqueue_media')) {
        //     ls_enqueue_media();
        // }

        Media::addJsDef(['PS_Modules' => ls_get_modules()]);
        ...
        ls_enqueue_script('kreaturamedia-ui', LS_VIEWS_URL . 'js/admin/km-ui.js', ['jquery'], LS_PLUGIN_VERSION);
        ls_enqueue_script('ls-admin-global', LS_VIEWS_URL . 'js/admin/ls-admin-common.js', ['jquery'], LS_PLUGIN_VERSION);
        ...
        // 3rd-party: Font Awesome
        ls_enqueue_style('ce-font-awesome', LS_VIEWS_URL . 'lib/font-awesome/css/font-awesome.min.css', false, '4.7.0');

        // 3rd-party: CodeMirror
        ls_enqueue_style('codemirror', LS_VIEWS_URL . 'css/codemirror/lib/codemirror.css', false, LS_PLUGIN_VERSION);
        ls_enqueue_script('codemirror', LS_VIEWS_URL . 'js/codemirror/lib/codemirror.js', ['jquery'], LS_PLUGIN_VERSION);
        ls_enqueue_style('codemirror-solarized', LS_VIEWS_URL . 'css/codemirror/theme/solarized.mod.css', false, LS_PLUGIN_VERSION);
        ls_enqueue_script('codemirror-syntax-css', LS_VIEWS_URL . 'js/codemirror/mode/css/css.js', ['jquery'], LS_PLUGIN_VERSION);
        ls_enqueue_script('codemirror-syntax-javascript', LS_VIEWS_URL . 'js/codemirror/mode/javascript/javascript.js', ['jquery'], LS_PLUGIN_VERSION);
        ls_enqueue_script('codemirror-foldcode', LS_VIEWS_URL . 'js/codemirror/addon/fold/foldcode.js', ['jquery'], LS_PLUGIN_VERSION);
        ls_enqueue_script('codemirror-foldgutter', LS_VIEWS_URL . 'js/codemirror/addon/fold/foldgutter.js', ['jquery'], LS_PLUGIN_VERSION);
        ls_enqueue_script('codemirror-brace-fold', LS_VIEWS_URL . 'js/codemirror/addon/fold/brace-fold.js', ['jquery'], LS_PLUGIN_VERSION);
        ls_enqueue_script('codemirror-active-line', LS_VIEWS_URL . 'js/codemirror/addon/selection/active-line.js', ['jquery'], LS_PLUGIN_VERSION);

        // Localize admin scripts
        include LS_ROOT_PATH . '/wp/scripts_l10n.php';
        Media::addJsDef(['LS_l10n' => $l10n_ls]);

        $controller = Tools::getValue('controller');

        // Sliders list page
        if ('AdminLayerSlider' === $controller && empty(${'_GET'}['action'])) {
            ls_enqueue_script('ls-admin-sliders', LS_VIEWS_URL . 'js/admin/ls-admin-sliders.js', ['jquery'], LS_PLUGIN_VERSION);
        ...
        // Slider & Transition Builder
            ...
            ls_register_script('layerslider-admin', LS_VIEWS_URL . 'js/admin/ls-admin-slider-builder.js', ['jquery', 'json2'], LS_PLUGIN_VERSION);

            // Slider Builder JS. Don't load automatically other than the Slider Builder
            if ('AdminLayerSlider' === $controller) {
                ls_enqueue_script('layerslider-admin');
            }

            // LayerSlider includes for preview
            ls_enqueue_script('layerslider', LS_VIEWS_URL . 'js/layerslider/layerslider.webshopworks.jquery.js', ['jquery'], LS_PLUGIN_VERSION);
            ls_enqueue_script('layerslider-transitions', LS_VIEWS_URL . 'js/layerslider/layerslider.transitions.js', false, LS_PLUGIN_VERSION);
            ls_enqueue_script('layerslider-tr-gallery', LS_VIEWS_URL . 'js/admin/layerslider.transition.gallery.js', ['jquery'], LS_PLUGIN_VERSION);
            ls_enqueue_style('layerslider', LS_VIEWS_URL . 'css/layerslider/layerslider.css', false, LS_PLUGIN_VERSION);
            ...
            ls_enqueue_script('layerslider-timeline', LS_VIEWS_URL . 'js/layerslider/plugins/timeline/layerslider.timeline.js', ['jquery'], LS_PLUGIN_VERSION);
            ...
            // LayerSlider Origami plugin
            ls_enqueue_script('layerslider-origami', LS_VIEWS_URL . 'js/layerslider/plugins/origami/layerslider.origami.js', ['jquery'], LS_PLUGIN_VERSION);
            ...
            // 3rd-party: MiniColor
            ls_enqueue_script('minicolor', LS_VIEWS_URL . 'js/minicolors/jquery.minicolors.js', ['jquery'], LS_PLUGIN_VERSION);
            ...
            ls_enqueue_script('air-datepicker', LS_VIEWS_URL . 'js/air-datepicker/air-datepicker.min.js', ['jquery'], '2.1.0en');

            // 3rd party: html2canvas
            ls_enqueue_script('html2canvas', LS_VIEWS_URL . 'js/html2canvas/html2canvas.min.js', ['jquery'], '1.0.0a9');
    ...
    // Transition builder
    if (false !== strpos($screen->base, 'ls-transition-builder')) {
        ls_enqueue_script('layerslider_tr_builder', LS_VIEWS_URL . 'js/admin/ls-admin-transition-builder.js', ['jquery'], LS_PLUGIN_VERSION);
    }

    // Revisions
    if (false !== strpos($screen->base, 'ls-revisions')) {
        ls_enqueue_style('ls-revisions', LS_VIEWS_URL . 'css/admin/revisions.css', false, LS_PLUGIN_VERSION);
        ls_enqueue_script('ls-revisions', LS_VIEWS_URL . 'js/admin/ls-admin-revisions.js', ['jquery'], LS_PLUGIN_VERSION);
    }

    // Skin editor
    if (false !== strpos($screen->base, 'ls-skin-editor') || false !== strpos($screen->base, 'ls-style-editor')) {
        ls_enqueue_style('ls-skin-editor', LS_VIEWS_URL . 'css/admin/skin.editor.css', false, LS_PLUGIN_VERSION);
    }

    // About page
    if (false !== strpos($screen->base, 'ls-about')) {
...
{
    // Get font list
    $fonts = ls_get_option('ls-google-fonts', []);
    $scripts = ls_get_option('ls-google-font-scripts', ['latin', 'latin-ext']);
    ...
    if ($fonts && is_array($fonts)) {
        ...
        if ($lsFonts) {
            ls_enqueue_style(
                'ls-google-fonts',
                'https://fonts.googleapis.com/css?family=' . implode('|', $lsFonts) . '&subset=' . implode(',', $scripts)
            );
        }
    }
}
...
{
    $str = '<meta name="generator" content="Powered by Creative Slider ' . LS_PLUGIN_VERSION . ' - Multi-Purpose, Responsive, Parallax, Mobile-Friendly Slider Module for PrestaShop.">' . PHP_EOL;
