...
function ls_preview_for_slider($slider = [])
{
    ...
    if (isset($slider['data']['layers'])) {
        foreach ($slider['data']['layers'] as $layer) {
            ...
            if (!empty($layer['properties']['background']) && '[image-url]' !== $layer['properties']['background']) {
...
function ls_parse_defaults($defaults = [], $raw = [])
{
    ...
    foreach ($defaults as $key => $default) {
        ...
        if (isset($default['props']['forceoutput'])) {
            ...
            if (true == $default['value'] && empty($raw[$phpKey])) {
                $ret[$retKey][$jsKey] = false;
            } elseif (false == $default['value'] && !empty($raw[$phpKey])) {
