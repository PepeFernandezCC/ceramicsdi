...
function lsOptionRow($type, $default, $current, $attrs = [], $trClasses = '', $forceOptionVal = false)
{
    ...
    if (!empty($default['advanced'])) {
        ...
        $wrapperStart = '<div><i class="dashicons dashicons-flag" data-help="' . ls__('Advanced option') . '"></i>';
        $wrapperEnd = '</div>';
    }
...
function lsGetInput($default, $current, $attrs = [], $return = false)
{
    // Markup
...
function lsGetCheckbox($default, $current, $attrs = [], $return = false)
{
    // Markup
    $el = LsQuery::newDocumentHTML('<input>');
    $attributes = [];
    ...
    if (true === $default['value'] && (!isset($current[$name]) || count($current) < 3)) {
        $attributes['checked'] = 'checked';
        $attributes['data-value'] = 'true';
    } elseif (isset($current[$name]) && false != $current[$name] && 'false' !== $current[$name]) {
        $attributes['checked'] = 'checked';
        $attributes['data-value'] = 'true';
    }
...
function lsGetSelect($default, $current, $attrs = [], $forceOptionVal = false, $return = false)
{
