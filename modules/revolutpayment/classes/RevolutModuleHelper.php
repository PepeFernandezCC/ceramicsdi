...
trait RevolutModuleHelper
{
    ...
    protected function addOrderMessage($id_new_order, $message)
    {
        ...
        if ($order_customer_thread) {
            ...
            $customer_thread->token = substr(Tools::getToken(false), 0, 12);
    ...
    public function collectLineItems($ps_order_id)
    {
        ...
        foreach ($products as $product) {
            ...
            if (empty($product_name) || empty($quantity_value) || empty($unit_price_amount) || empty($total_amount)) {
                continue;
            }
    ...
    public function updateRevolutOrderLineItemsAndShippingData($ps_order_id, $revolut_order_id)
    {
        try {
            ...
            if (!empty($params)) {
                RevolutApiHelper::updateRevolutOrder($revolut_order_id, $params);
            }
        } catch (Exception $e) {
    ...
    /**
     * Returns pay by bank method capability withing merchant
     *
     * @return bool
     */
    public function isPayByBankAllowed()
    {
        $merchant_features = RevolutApiHelper::getMerchantFeatures();

        return in_array(self::$openbankingForEurFlagName, $merchant_features);
    }

    /**
     * Returns cardholder name field availability
     *
     * @return bool
     */
    public function isCardHolderNameFieldEnabled()
    {
        if (!$this->cardHolderNameFieldEnabled) {
            return false;
        }

        if ($this->checkoutWidgetDisplayType === (string) 3) {
            return false;
        }
