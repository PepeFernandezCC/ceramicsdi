...
class DeliverypricecalculatorPriceModuleFrontController extends ModuleFrontController
{
    ...
    public function initContent()
    {
        ...
        $response = $this->calculateShippingPrice($realCart, $products, $fakeCustomer, $address, $id_country, $id_state, $showTaxes);

        $this->ajaxDie(json_encode($response));
    }

    /**
     * A partir de aqui existe un carrito temporal (ademas de la direccion ya
     * creada en initContent()) que SIEMPRE hay que borrar, incluso si algo
     * lanza una excepcion a mitad de camino (p.ej.
     * Carrier::getCheapestDeliveryOptionByCart fallando porque un
     * transportista externo no responde/se cuelga). El try/finally aquí
     * dentro sustituye a repetir el borrado en cada rama de error - antes
     * bastaba con que una sola ruta se olvidara de borrar (o con que algo
     * lanzara una excepción en vez de simplemente devolver false) para que
    ...
    private function calculateShippingPrice(Cart $realCart, array $products, Customer $fakeCustomer, Address $address, $id_country, $id_state, $showTaxes)
    {
        ...
        try {
            ...
            PrestaShopLogger::addLog(
                'deliverypricecalculator: fallo calculando coste de envío - ' . $e->getMessage(),
                3
            );

            return ['error' => 'No se pudo calcular el coste de envío'];
        } finally {
            // Se ejecuta siempre: cualquier "return" de arriba, o una
            // excepcion no prevista. Nunca deja el carrito/direccion
