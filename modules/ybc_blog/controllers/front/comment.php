...
class Ybc_blogCommentModuleFrontController extends ModuleFrontController
{
	...
	public function init()
	{
        ...
        if($module->friendly && Tools::strpos($_SERVER['REQUEST_URI'],'/module/ybc_blog') !==false)
        {
            $module->redirect($module->getLink('comment'));
        }
        parent::canonicalRedirection($module->getLink('comment'));
	}
    public function getAlternativeLangsUrl()
    {
        ...
        foreach ($languages as $lang) {
            $alternativeLangs[$lang['language_code']] = $module->getLanguageLink($lang['id_lang']);
        }
        return $alternativeLangs;
    }
    public function _initContent()
    {
        /** @var Ybc_blog $module */
        $module = Module::getInstanceByName('ybc_blog');
        $totalRecords= Ybc_blog_comment_class::countCommentsWithFilter($this->context, ' AND bc.approved=1');
        ...
        if($start < 0)
            $start = 0;
        $posts = Ybc_blog_comment_class::getCommentsWithFilter($this->context, ' AND bc.approved=1','bc.datetime_added DESC,',$start,$paggination->limit);;
        if($posts)
        {
            foreach($posts as &$post)
            {
                ...
                $post['comments_num'] = Ybc_blog_comment_class::countCommentsWithFilter($this->context, ' AND bc.id_post='.(int)$post['id_post'].' AND approved=1');
                $post['liked'] = $module->isLikedPost($post['id_post']);
                ...
                if($post['id_user'])
                {
                    $customerinfo = Ybc_blog_post_employee_class::getInformationByID($this->context->language->id, $post['id_user']);
	...
	public function initContent()
	{
        ...
        else
        {
            ...
            if(!$module->isCached('comment_list.tpl',$module->_getCacheId($page)))
            {
                $this->_initContent();
            }
            $this->context->smarty->assign(
                array(
                    'commnet_list_content' => $this->module->display($this->module->getLocalPath(),'comment_list.tpl',$module->_getCacheId($page)),
