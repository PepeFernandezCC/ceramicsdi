...
class Ybc_blog_post_employee_class extends ObjectModel
{
    ...
    public static function getAuthors($context)
    {
        ...
        if ($admin_authors) {
            foreach ($admin_authors as &$admin_author)
                $admin_author['link'] = $context->link->getAdminLink('AdminEmployees') . '&id_employee=' . (int)$admin_author['id_employee'] . '&updateemployee';
        }
    ...
    public static function getEmployeesFilter($context, $filter = false, $sort = false, $start = false, $limit = false, $having = '')
    {
        ...
        LEFT JOIN `" . _DB_PREFIX_ . "ybc_blog_employee_lang` bel ON (bel.id_employee_post=be.id_employee_post AND bel.id_lang='" . (int)$context->language->id . "')
        LEFT JOIN `" . _DB_PREFIX_ . "profile` p ON (e.id_profile=p.id_profile)
        LEFT JOIN `" . _DB_PREFIX_ . "profile_lang` pl ON (p.id_profile=pl.id_profile AND pl.id_lang='" . (int)$context->language->id . "')
        LEFT JOIN `" . _DB_PREFIX_ . "ybc_blog_post` bp ON (bp.added_by=e.id_employee AND bp.is_customer=0)
        LEFT JOIN `" . _DB_PREFIX_ . "ybc_blog_post_shop` bps ON (bps.id_post=bp.id_post AND bps.id_shop='" . (int)$context->shop->id . "')
        WHERE 1 " . ((string)$filter ? : '') . "
        GROUP BY e.id_employee
        HAVING (1 " . ((string)$having ? : ' ') . " )
        " . ($sort ? ' ORDER BY ' . $sort : '') . ($start !== false && $limit ? " LIMIT " . (int)$start . ", " . (int)$limit : "");
        return Db::getInstance()->executeS($sql);
    }

    public static function getCustomersFilter($context, $filter = false, $sort = false, $start = false, $limit = false, $having = '')
    {
        $group_author = Configuration::get('YBC_BLOG_GROUP_CUSTOMER_AUTHOR');
        if ($group_author) {
            $group_author = explode(',', $group_author);
            ...
            LEFT JOIN `" . _DB_PREFIX_ . "ybc_blog_post_shop` bps ON (bp.id_post=bps.id_post AND bps.id_shop='" . (int)$context->shop->id . "')
            WHERE c.id_shop='" . (int)$context->shop->id . "' AND  g.id_group in (" . implode(',', array_map('intval', $group_author)) . ") " . ((string)$filter ? : '') . "
            GROUP BY c.id_customer
            HAVING (1 " . ((string)$having ? $having : ' ') . " )
            " . ($sort ? ' ORDER BY ' . $sort : '') . ($start !== false && $limit ? " LIMIT " . (int)$start . ", " . (int)$limit : "");
            return Db::getInstance()->executeS($sql);
        }
        return array();
    }
    public static function countEmployeesFilter($context, $filter, $having = '')
    {
    ...
    public static function countCustomersFilter($context, $filter, $having = '')
    {
        $group_author = Configuration::get('YBC_BLOG_GROUP_CUSTOMER_AUTHOR');
        if ($group_author) {
            $group_author = explode(',', $group_author);
    ...
    public static function getAuthorById($id_lang, $id_author, $is_customer = 0)
    {
        if ($is_customer) {
    ...
    public static function checkPermistionPost($context, $id_post = 0, $permistion = '')
    {
        $id_employee_post = (int)Ybc_blog_post_employee_class::getIdEmployeePostById($context->customer->id, true);
        $employeePost = new Ybc_blog_post_employee_class($id_employee_post);
        if (!$id_employee_post || $employeePost->status == 1) {
            $privileges = explode(',', Configuration::get('YBC_BLOG_AUTHOR_PRIVILEGES'));
            if (in_array($permistion, $privileges)) {

                if ($permistion == 'edit_blog' || $permistion == 'delete_blog')
                    return Db::getInstance()->getRow('SELECT * FROM `' . _DB_PREFIX_ . 'ybc_blog_post` WHERE id_post="' . (int)$id_post . '" AND added_by="' . (int)$context->customer->id . '" AND is_customer=1');
                else
                    return true;
    ...
    public static function checkPermisionComment($context, $action = 'edit', $id_comment = 0, $tabmanagament = 'comment')
    {
        if (!isset($context->customer) || !$context->customer->isLogged())
            return false;
        $privileges = Configuration::get('YBC_BLOG_AUTHOR_PRIVILEGES');
        $ok = true;
        if (!$privileges || !self::checkGroupAuthor($context))
            $ok = false;
        elseif (self::checkGroupAuthor($context)) {
            ...
            if ($tabmanagament == 'comment' || $action != '') {
                ...
                elseif (!in_array('manage_comments', $privileges) && $action != 'reply')
                    ...
                    WHERE c.id_comment="' . (int)$id_comment . '" AND p.added_by="' . (int)$context->customer->id . '" AND p.is_customer=1';
                    $ok = Db::getInstance()->getValue($sql);
                }
            }
        }
        if (!$ok && Db::getInstance()->getValue('SELECT id_comment FROM `' . _DB_PREFIX_ . 'ybc_blog_comment` WHERE id_user="' . (int)$context->customer->id . '" AND id_comment=' . (int)$id_comment)) {
    ...
    public static function checkGroupAuthor($context)
    {
    ...
    public static function authValid($customer)
    {
        ...
        if ($customer->id && ($authorGroups = Configuration::get('YBC_BLOG_GROUP_CUSTOMER_AUTHOR'))) {
            $authorGroups = explode(',', $authorGroups);
    ...
    public static function getCountAuthor($id_shop, $active = true)
    {
    ...
    public static function getListAuthorPost($id_shop, $start, $limit = false)
    {
    ...
    public static function getInformationByID($id_lang, $id, $is_customer = true)
    {
        if ($is_customer) {
    ...
    public static function getPosts($context, $id, $is_customer = 1)
    {
