...
class Ybc_blog_post_class extends ObjectModel
{
    ...
    public function __construct($id_item = null, $id_lang = null, $id_shop = null)
    {
        ...
        if ($this->id) {
            ...
            if (!Cache::isStored($cache_key)) {
                $this->id_shop = (int)Db::getInstance()->getValue('SELECT id_shop FROM `' . _DB_PREFIX_ . 'ybc_blog_post_shop` where id_post= ' . (int)$this->id);
    ...
    public static function getAuthorByIdPost($context, $id_post)
    {
        ...
        if ($post) {
            ...
            ');
                if ($author)
                    $author['link'] = $context->link->getAdminLink('AdminCustomers') . '&updatecustomer&id_customer=' . $author['id_customer'];
            } else {
            ...
            ');
                if ($author)
                    $author['link'] = $context->link->getAdminLink('AdminEmployees') . '&id_employee=' . (int)$author['id_employee'] . '&updateemployee';
            }
            if ($author) {
                return $author;
            }
    ...
    public static function _deletePost($id_post, $sort = true)
    {
        ...
        if (Validate::isLoadedObject($post)) {
            if ($post->delete()) {
                ...
                if ($sort) {
                    $posts = Db::getInstance()->executeS('SELECT * FROM `' . _DB_PREFIX_ . 'ybc_blog_post` p, `' . _DB_PREFIX_ . 'ybc_blog_post_shop` ps  WHERE p.id_post=ps.id_post AND  ps.id_shop="' . (int)$post->id_shop . '" order by p.sort_order asc');
                    if ($posts) {
    ...
    public static function getPostsByIdCategory($context, $id_category, $id_lang = false, $enabled = false)
    {
        ...
        return self::getPostsWithFilter($context, $filter, $sort, false, false, false, $id_category);
    }

    protected static $authors = array();

    public static function getPostsWithFilter($context, $filter = false, $sort = false, $start = false, $limit = false, $fontend = true, $id_category = 0)
    {
        $req = "SELECT p.*,pc.id_category, pl.image,pl.thumb, pl.title, pl.description, pl.short_description, pl.meta_keywords, pl.meta_description,pl.url_alias,e.firstname, e.lastname,pc.position,count(pcm.id_comment) as total_comment,IFNULL(ybe.status,1) as status
            ...
            INNER JOIN `" . _DB_PREFIX_ . "ybc_blog_post_shop` ps ON (p.id_post=ps.id_post AND ps.id_shop='" . (int)$context->shop->id . "')
            ...
            foreach ($posts as  &$post) {
                ...
                if ($post['is_customer']) {
                    ...
                    if (Validate::isLoadedObject($customer)) {
                        $link_author = $module->getlink('blog', array('id_author' => $post['added_by'], 'is_customer' => $post['is_customer']));
                        if (!($author && isset($author['name']) && $author['name']))
                            $post['name_author'] = $customer->firstname . ' ' . $customer->lastname;
                        else
                            $post['name_author'] = $author['name'];
                        $post['name_author'] = $module->displayText($post['name_author'], 'a', null, null, $link_author, true) . ' (Role: customer' . ($author && $author['status'] <= 0 ? ', suspend' : '') . ')';
                    } else
                        $post['name_author'] = '';
                    ...
                    if (Validate::isLoadedObject($employee)) {
                        $link_author = $module->getlink('blog', array('id_author' => $post['added_by']));
                        ...
                        $profile = new Profile($id_profile, $context->language->id);
                        $post['name_author'] = $module->displayText($post['name_author'], 'a', null, null, $link_author, true) . ' (Role: ' . $profile->name . ($author && $author['status'] <= 0 && $id_profile != 1 ? ', suspend' : '') . ')';
                    } else
                        $post['name_author'] = '';
    ...
    public static function getOnlyCategoryBlog($id_post, $id_lang)
    {
    ...
    public static function getProductsByIDs($context, $id_products)
    {
        if ($id_products) {
            $sql = 'SELECT p.`id_product`, pl.`link_rewrite`, p.`reference`, pl.`name`
			FROM `' . _DB_PREFIX_ . 'product` p
            ' . Shop::addSqlAssociation('product', 'p') . '
			LEFT JOIN `' . _DB_PREFIX_ . 'product_lang` pl ON (p.`id_product` = pl.`id_product` and pl.id_lang="' . (int)$context->language->id . '" AND pl.id_shop=product_shop.id_shop)
            WHERE p.`id_product` IN (' . implode(',', array_map('intval', $id_products)) . ') GROUP BY p.id_product';
            $product_list = Db::getInstance()->executeS($sql);
            if ($product_list) {
                $type_image = Ybc_blog::getFormattedName('small', $context);
                foreach ($product_list as &$product) {
                    ...
                    $product['link_image'] = $context->link->getImageLink($product['link_rewrite'], $id_image, $type_image);
                    $product['link'] = $context->link->getProductLink($product['id_product'], null, null, null, null, null, Product::getDefaultAttribute($product['id_product']));
                }
            }
            return $product_list;
        }
    ...
    public static function getTagsByIdPost($id_post, $id_lang = false)
    {
        ...
        if ($tags) {
            ...
            $module = Module::getInstanceByName('ybc_blog');
            foreach ($tags as &$tag) {
                $tag['link'] = $module->getLink('blog', array('tag' => urlencode($tag['tag'])));
            }
        }
        return $tags;
    }
    ...
    public static function getTags($context, $limit = 20)
    {
    ...
    public static function checkUrlAliasExists($url_alias, $id_post, $id_shop)
    {
        return Db::getInstance()->getValue('SELECT ps.id_post FROM `' . _DB_PREFIX_ . 'ybc_blog_post_lang` pl
        INNER JOIN `' . _DB_PREFIX_ . 'ybc_blog_post_shop` ps ON ps.id_post= pl.id_post AND ps.id_shop="' . (int)$id_shop . '"
        WHERE pl.url_alias ="' . pSQL($url_alias) . '" AND ps.id_post!="' . (int)$id_post . '"');
    }

    public static function getMaxOrder($id_shop)
    {
    ...
    public static function countPostsWithFilter($context, $filter, $fontend = true)
    {
        $req = "SELECT DISTINCT p.*, pl.title, pl.description
            ...
            LEFT JOIN `" . _DB_PREFIX_ . "ybc_blog_employee` ybe ON ((ybe.id_employee=c.id_customer AND ybe.is_customer=1) OR (ybe.id_employee=e.id_employee AND ybe.is_customer=0))
            LEFT JOIN `" . _DB_PREFIX_ . "ybc_blog_comment` pcm on (pcm.id_post=p.id_post)
            WHERE " . ($fontend ? "(p.enabled=1 OR p.enabled=-1) AND (ybe.status>=0 OR ybe.status is NULL OR e.id_profile=1) AND " : "") . "pl.id_lang = " . (int)$context->language->id . ((string)$filter ? : '');
    ...
    public static function getPostAlias($id_post, $id_lang)
    {
        if (!$id_post)
            return false;
    ...
    public static function getBlogPositiveAuthor($context)
    {
        ...
        if ($authors) {
            ...
            foreach ($authors as &$author) {
                if ($author['is_customer']) {
                    ...
                    LEFT JOIN `' . _DB_PREFIX_ . 'ybc_blog_employee_lang` bel ON (be.id_employee_post=bel.id_employee_post AND bel.id_lang="' . (int)$context->language->id . '")
                    ...
                    $author['link'] = $module->getLink('blog', array('id_author' => $author['added_by'], 'is_customer' => 1, 'alias' => Tools::str2url($information['name'])));
                    if (isset($information['avata']) && $information['avata'])
                        $author['avata'] = $context->link->getMediaLink(_PS_YBC_BLOG_IMG_ . 'avata/' . $information['avata']);
                    else
                        $author['avata'] = $context->link->getMediaLink(_PS_YBC_BLOG_IMG_ . 'avata/' . (Configuration::get('YBC_BLOG_IMAGE_AVATA_DEFAULT') ? Configuration::get('YBC_BLOG_IMAGE_AVATA_DEFAULT') : 'default_customer.png'));
                } else {
                    $information = Db::getInstance()->getRow('
                    ...
                    if (!$information['name'])
                        $information['name'] = $information['firstname'] . ' ' . $information['lastname'];
                    $author['information'] = $information;
                    $author['link'] = $module->getLink('blog', array('id_author' => $author['added_by'], 'is_customer' => 0, 'alias' => Tools::str2url($information['name'])));
                    if (isset($information['avata']) && $information['avata'])
                        $author['avata'] = $context->link->getMediaLink(_PS_YBC_BLOG_IMG_ . 'avata/' . $information['avata']);
                    else
                        $author['avata'] = $context->link->getMediaLink(_PS_YBC_BLOG_IMG_ . 'avata/' . (Configuration::get('YBC_BLOG_IMAGE_AVATA_DEFAULT') ? Configuration::get('YBC_BLOG_IMAGE_AVATA_DEFAULT') : 'default_customer.png'));
    ...
    public static function getIDPostByUrlAlias($id_shop, $url_alias, $id_lang = 0)
    {
        ...
        if (!Cache::isStored($cache_key)) {
            ...
            INNER JOIN `' . _DB_PREFIX_ . 'ybc_blog_post_shop` ps ON (p.id_post=ps.id_post AND ps.id_shop="' . (int)$id_shop . '")
    ...
    public static function getBlogRssAuthor($id_lang)
    {
        $employees = Db::getInstance()->executeS(
            ...
            LEFT JOIN `' . _DB_PREFIX_ . 'ybc_blog_employee_lang` bel ON (be.id_employee_post= bel.id_employee_post AND bel.id_lang="' . (int)$id_lang . '") 
        ...
        if ($employees) {
            foreach ($employees as &$employee) {
                $employee['name'] = $employee['name'] ? : $employee['firstname'] . ' ' . $employee['lastname'];
                $employee['link'] = $module->getLink('rss', array('id_author' => $employee['id_employee'], 'is_customer' => 0, 'alias' => Tools::str2url($employee['name'])));
            }
        }
        $group_authors = Configuration::get('YBC_BLOG_GROUP_CUSTOMER_AUTHOR');
        if ($group_authors) {
            $group_authors = explode(',', $group_authors);
            ...
            if ($customers) {
                foreach ($customers as &$customer) {
                    $customer['name'] = $customer['name'] ? : $customer['firstname'] . ' ' . $customer['lastname'];
                    $customer['link'] = $module->getLink('rss', array('id_author' => $customer['id_customer'], 'is_customer' => 1, 'alias' => Tools::str2url($customer['name'])));
                }
            }
    ...
    public static function getFirstCategory($id_post, $id_shop)
    {
        $sql = 'SELECT c.id_category FROM `' . _DB_PREFIX_ . 'ybc_blog_category` c
                ...
                WHERE pc.id_post="' . (int)$id_post . '" AND cs.id_shop="' . (int)$id_shop . '" ORDER BY c.sort_order ASC';
        return Db::getInstance()->getValue($sql);
    }

    public static function getPostRelatedByIdProduct($id_product, $context)
    {
        ...
        INNER JOIN `' . _DB_PREFIX_ . 'ybc_blog_post_shop` ps ON (ps.id_post = p.id_post AND ps.id_shop="' . (int)$context->shop->id . '")
        LEFT JOIN `' . _DB_PREFIX_ . 'ybc_blog_post_lang` pl ON (pl.id_post= p.id_post AND pl.id_lang="' . (int)$context->language->id . '")';
        return Db::getInstance()->executeS($sql);
    }

    public static function getRelatedPosts($id_shop, $id_post, $tags, $id_lang = false)
    {
        ...
        $sql = "SELECT pl.title, pl.short_description,pl.description,pl.image,pl.thumb, p.*
            ...
            WHERE ps.id_shop='" . (int)$id_shop . "' AND  p.enabled=1 AND (t.tag IN ('" . implode("','", array_map('pSQL', $tagElements)) . "') OR pc.id_category IN (SELECT id_category FROM `" . _DB_PREFIX_ . "ybc_blog_post_category` WHERE id_post=" . (int)$id_post . ")) AND p.id_post != " . (int)$id_post . "
            GROUP BY pl.id_post
    ...
    public static function getPostsByIdProduct($context, $id_product, $limit = 20)
    {
        $sql = "SELECT p.*,pl.* FROM `" . _DB_PREFIX_ . "ybc_blog_post` p
        INNER JOIN `" . _DB_PREFIX_ . "ybc_blog_post_shop` ps ON (ps.id_post= p.id_post AND id_shop='" . (int)$context->shop->id . "')
        LEFT JOIN `" . _DB_PREFIX_ . "ybc_blog_post_lang` pl ON (p.id_post=pl.id_post)
        ...
        WHERE p.enabled=1 AND FIND_IN_SET('" . (int)$id_product . "', REPLACE(p.exclude_products,'-', ','))=0 AND (pp.id_product ='" . (int)$id_product . "' OR cp.id_product='" . (int)$id_product . "') AND pl.id_lang=" . (int)$context->language->id . ' GROUP BY p.id_post LIMIT 0,' . (int)$limit;
        return Db::getInstance()->executeS($sql);
    }

    public static function getBlogArchives($id_shop)
    {
        ...
        if ($years) {
            ...
            foreach ($years as &$year) {
                ...
                $year['link'] = $module->getLink('blog', array('year' => $year['year_add']));
                if ($year['months']) {
                    foreach ($year['months'] as &$month) {
                        $month['link'] = $module->getLink('blog', array('month' => $month['month_add'], 'year' => $year['year_add']));
                        $month['month_add'] = $module->getMonthName($month['month_add']);
                    }
                }
            }
        }
    ...
    public static function getPostByAuthor($id_shop, $id_author, $is_customer = 0)
    {
    ...
    public static function deleteAllPostCustomerByIdAuthor($id_shop, $id_author)
    {
    ...
    public static function logViewCustomer($id_post, $browser, $id_customer)
    {
        $ip = Tools::getRemoteAddr();
        if (!Db::getInstance()->getRow('SELECT * FROM `' . _DB_PREFIX_ . 'ybc_blog_log_view` WHERE ' . ($id_customer ? 'id_customer="' . (int)$id_customer . '"' : 'ip="' . pSQL($ip) . '" AND id_customer=0') . ' AND DAY(datetime_added) ="' . pSQL(date('d')) . '" AND MONTH(datetime_added) ="' . pSQL(date('m')) . '" AND YEAR(datetime_added) ="' . pSQL(date('Y')) . '" AND id_post=' . (int)$id_post)) {
            Db::getInstance()->execute('INSERT INTO `' . _DB_PREFIX_ . 'ybc_blog_log_view`(ip,id_post,browser,id_customer,datetime_added) VALUES ("' . pSQL($ip) . '","' . (int)$id_post . '","' . pSQL($browser) . '","' . (int)$id_customer . '","' . pSQL(date('Y-m-d H:i:s')) . '")');
            Db::getInstance()->execute('UPDATE `' . _DB_PREFIX_ . 'ybc_blog_post` SET click_number= click_number+1 WHERE id_post=' . (int)$id_post);
        }
    }

    public function addLike($browser, $id_customer)
    {
        $ip = Tools::getRemoteAddr();
        $sql = 'INSERT INTO `' . _DB_PREFIX_ . 'ybc_blog_log_like`(ip,id_post,browser,id_customer,datetime_added) VALUES ("' . pSQL($ip) . '","' . (int)$this->id . '","' . pSQL($browser) . '","' . (int)$id_customer . '","' . pSQL(date('Y-m-d H:i:s')) . '")';
    ...
    public function unLike($id_customer)
    {
        if ($id_customer) {
            Db::getInstance()->execute('DELETE FROM `' . _DB_PREFIX_ . 'ybc_blog_log_like` WHERE id_customer="' . (int)$id_customer . '"');
        }
        if ($this->likes > 0)
            $this->likes--;
        $this->update();
    ...
    public static function clearAllViewLog($id_shop)
    {
        return Db::getInstance()->execute('DELETE FROM `' . _DB_PREFIX_ . 'ybc_blog_log_view` WHERE id_post IN (SELECT id_post FROM `' . _DB_PREFIX_ . 'ybc_blog_post_shop` WHERE id_shop=' . (int)$id_shop . ')');
    }

    public static function clearAllLikeLog($id_shop)
    {
        Db::getInstance()->execute('DELETE FROM `' . _DB_PREFIX_ . 'ybc_blog_log_like` WHERE id_post IN (SELECT id_post FROM `' . _DB_PREFIX_ . 'ybc_blog_post_shop` WHERE id_shop=' . (int)$id_shop . ')');
    }
    ...
    public static function getCountLogViews($id_shop)
    {
    ...
    public static function getLogViews($context, $start, $limit)
    {
    ...
    public static function getCountLogLikes($id_shop)
    {
    ...
    public static function getLogLikes($context, $start, $limit)
    {
    ...
    public static function getCountView($id_shop, $year = '', $month = '', $day = '', $id_post = 0)
    {
        return Db::getInstance()->getValue('SELECT COUNT(*) FROM `' . _DB_PREFIX_ . 'ybc_blog_log_view` l
        INNER JOIN `' . _DB_PREFIX_ . 'ybc_blog_post_shop` ps ON l.id_post=ps.id_post AND ps.id_shop=' . (int)$id_shop . ' 
    ...
    public static function getCountLike($id_shop, $year = '', $month = '', $day = '', $id_post = 0)
    {
    ...
    public static function getCountComment($id_shop, $year = '', $month = '', $day = '', $id_post = 0)
    {
        return Db::getInstance()->getValue('SELECT COUNT(*) FROM `' . _DB_PREFIX_ . 'ybc_blog_comment` c 
        INNER JOIN `' . _DB_PREFIX_ . 'ybc_blog_post_shop` ps ON c.id_post=ps.id_post AND ps.id_shop ="' . (int)$id_shop . '"
    ...
    public static function getProductInfo($context, $id_product)
    {
        ...
        $pinfo['price'] = Tools::displayPriceSmarty(['price'=>$price,'currency'=> $context->currency->id], $context->smarty);
        $pinfo['old_price'] = Tools::displayPriceSmarty(['price'=> $oldPrice,'currency'=> $context->currency->id], $context->smarty);
        $pinfo['discount_percent'] = (($oldPrice - $price) > 0 ? round(($oldPrice - $price) / $oldPrice * 100) : 0);
        $pinfo['discount_amount'] = Tools::displayPriceSmarty(['price'=> $discount, 'currency'=> $context->currency->id], $context->smarty);
        ...
        $pinfo['img_url'] = $context->link->getImageLink($product->link_rewrite, $id_image, Ybc_blog::getFormattedName('home', $context));
        $pinfo['link'] = $context->link->getProductLink($product, null, null, null, null, null, $product->cache_default_attribute);
        return $pinfo;
    }

    public static function getRelatedProductByProductsStr($context, $id_post, $exclude_products)
    {
        if (Configuration::get('YBC_BLOG_SHOW_RELATED_PRODUCTS')) {
            ...
            if ($ids) {
                foreach ($ids as $pid) {
                    if ($pid && !in_array($pid, $exclude_ids) && !isset($products[$pid])) {
                        $product = self::getProductInfo($context, (int)$pid);
                        if ($product)
            ...
            INNER JOIN `' . _DB_PREFIX_ . 'product_shop` ps ON (p.id_product= ps.id_product AND ps.id_shop = "' . (int)$context->shop->id . '")
            ...
            if ($category_products) {
                foreach ($category_products as $p) {
                    if (!isset($products[$p['id_product']])) {
                        $product = self::getProductInfo($context, (int)$p['id_product']);
                        if ($product)
    ...
    public static function getPostsByQuery($context, $query)
    {
        $sql = 'SELECT pl.*,p.* FROM `' . _DB_PREFIX_ . 'ybc_blog_post` p
        INNER JOIn `' . _DB_PREFIX_ . 'ybc_blog_post_shop` ps ON (ps.id_post= p.id_post AND ps.id_shop="' . (int)$context->shop->id . '")
        LEFT JOIN `' . _DB_PREFIX_ . 'ybc_blog_post_lang` pl ON (p.id_post = pl.id_post  AND pl.id_lang="' . (int)$context->language->id . '")
