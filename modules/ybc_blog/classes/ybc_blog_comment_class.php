...
class Ybc_blog_comment_class extends ObjectModel
{
    ...
    public static function getCommentsWithFilter($context, $filter = false, $sort = false, $start = false, $limit = false,$fontend=true)
    {
        $req = "SELECT bc.*,pl.description,pl.short_description,pl.thumb,pl.title
            ...
            ORDER BY ".((string)$sort ? : '')." bc.id_comment desc " . ($start !== false && $limit ? " LIMIT ".(int)$start.", ".(int)$limit : "");
        ...
        if($comments)
        {
            ...
            foreach($comments as &$comment)
            {
                ...
                {
                    $customer= Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'customer` WHERE id_shop="'.(int)$context->shop->id.'" AND  id_customer='.(int)$comment['replied_by']);
                ...
                if(Ybc_blog_post_employee_class::checkPermisionComment($context, 'edit',$comment['id_comment']))
                    $comment['url_edit'] = $module->getLink('blog',array('id_post'=>$comment['id_post'],'edit_comment'=>$comment['id_comment']));
                if(Ybc_blog_post_employee_class::checkPermisionComment($context, 'delete',$comment['id_comment']))
                    $comment['url_delete'] = $context->link->getModuleLink('ybc_blog','managementcomments',array('deletecomment'=>1,'id_comment'=>$comment['id_comment']));
    ...
    public static function countCommentsWithFilter($context, $filter = false,$fontend=true)
    {
    ...
    public static function getCommentById($id_comment, $id_lang)
    {
        ...
        LEFT JOIN `"._DB_PREFIX_."ybc_blog_post_lang` pl ON bc.id_post = pl.id_post AND pl.id_lang=".(int)$id_lang."
        WHERE bc.id_comment=".(int)$id_comment."
        ");
    }
    protected static $isLikes = array();
    public static function checkCustomerIsLikePost($id_post, $id_customer)
    {
        if(!isset(self::$isLikes[$id_post]))
            self::$isLikes[$id_post] = Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'ybc_blog_log_like` WHERE id_customer="'.(int)$id_customer.'" AND id_post="'.(int)$id_post.'"');
