...
class Ybc_blog_defines extends Module
{
    ...
    public $alias;
    public $friendly;
    public function getCustomerSettings($id_lang)
    {
        if(!isset(self::$customer_settings))
        {
            self::$customer_settings = array(
                'YBC_BLOG_ALLOW_CUSTOMER_AUTHOR' => array(
                    ...
                    'type' => 'switch',
                    'default' => 0,
                    'desc' => $this->l('A blog management area named "My blog posts" will be available in “My account” on the front office when the authors log into their account','ybc_blog_defines'),
                    'values' => array(
                        array(
                            'id' => 'active_on',
                            'value' => 1,
        ...
        if(!isset(self::$configs_image))
        {
            self::$configs_image = array(
                ...
                'YBC_BLOG_IMAGE_AVATA_DEFAULT' => array(
                    ...
                    'type' => 'file',
                    'col' => 9,
                    'desc' => sprintf($this->l('Accepted formats: jpg, jpeg, png, gif. Limit: %dMb. Recommended: 100px x 100px', 'ybc_blog_defines'),Configuration::get('PS_ATTACHMENT_MAXIMUM_SIZE')),
                ),
            );
        }
        ...
        if(!isset(self::$configs_global))
        {
            self::$configs_global = array(
                ...
                'YBC_BLOG_CAPTCHA_SECRET_KEY' => array(
                    ...
                    'tab' => 'general',
                    'required2'=>true,
                    'desc' => htmlspecialchars_decode($this->l('<a href="https://prestahero.com/help-center/general-configuration/290-create-recaptcha-keys-google-cloud" target="_blank">How to get your reCAPTCHA keys</a>', 'ybc_blog_defines')),
                ),
                ...
                'YBC_BLOG_ENABLE_ESTIMATED_READING_TIME' => array(
                    'label'=> $this->l('Show estimated reading time','ybc_blog_defines'),
                    'type'=>'switch',
                    'tab' => 'general',
                    'desc' => $this->l('Enable or disable showing estimated reading time at the beginning of each blog post.','ybc_blog_defines'),
                    'values' => array(
                        array(
                            'id' => 'YBC_BLOG_ENABLE_ESTIMATED_READING_TIME_on',
                            'value' => 1,
                            'label' => $this->l('Yes','ybc_blog_defines')
                        ),
                        array(
                            'id' => 'YBC_BLOG_ENABLE_ESTIMATED_READING_TIME_off',
                            'value' => 0,
                            'label' => $this->l('No','ybc_blog_defines')
                        )
                    ),
        ...
        if(!isset(self::$configs_email))
        {
            self::$configs_email = array(
                'YBC_BLOG_ALERT_EMAILS' => array(
                    'label' => $this->l('Admin email(s) for notifications','ybc_blog_defines'),
                    'type' => 'text',
                    'desc' => $this->l('Enter the email addresses of administrators who should receive notifications about new posts, comments, etc. Separate multiple emails with a comma (,). Some notifications may also be sent to other users, such as customers or authors, depending on the context.','ybc_blog_defines'),
                ),
            );
        }
    ...
    public static function getGroups($id_lang, $list_id=false)
    {
    ...
    public static function getProducts($id_lang, $query,$excludeIds=false,$excludeVirtuals=false,$exclude_packs=false)
    {
    ...
    public static function getInstance($context)
    {
        if (!isset(self::$instance)) {
            self::$instance = new Ybc_blog_defines($context);
        }
    ...
    public static function upgrade_module_2_0_1($context)
    {
        $sqls=array();
        ...
        if($categories)
        {
            foreach($categories as $category)
            {
                foreach($shops as $shop)
                {
                    ...
                    if($shop['id_shop']==$context->shop->id)
                    {
    ...
    public static function initData($context)
    {
