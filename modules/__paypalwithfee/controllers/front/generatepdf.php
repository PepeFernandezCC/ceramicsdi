...
class PayPalwithFeeGeneratepdfModuleFrontController extends ModuleFrontControllerCore
{
    public function init()
    {
        ...
        if (Tools::getValue('action') == 'IsOrderPpwf' &&
            Tools::getValue('ajax') && Tools::getValue('token') == Tools::getToken(false)) {
        ...
        if ($this->context->customer && $this->context->customer->isLogged() || $this->context->employee) {
            $id_order = Tools::getValue('id_order');
    ...
    public function ajaxProcessIsOrderPpwf()
    {
        ...
        // Add retrocompatibility for older products
        if ($order->module == $this->module->name) {
            foreach (OrderDetail::getList($id_order) as $key => $value) {
                unset($key);
            ...
            if (!$hasFee) {
                $this->ajaxDie(json_encode(array(
                            'is_ppwf' => true,
                            'id_order' => $id_order,
                            'href' => 'index.php?fc=module&module=paypalwithfee&controller=generatepdf&id_order=' .
                                $id_order,
                )));
            }
        } else {
    ...
    protected function getTaxTabContent($order, $invoice, $html_template_invoice, $fee)
    {
        ...
        return $html_template_invoice->smarty->fetch(
            _PS_MODULE_DIR_ . '/paypalwithfee/views/templates/admin/invoice.tax-tab.tpl'
        );
    }
