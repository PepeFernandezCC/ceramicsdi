...
class EtsAbancartIndexCustomer
{
    ...
    public static function getLastVisited($module, $context, $ip_address = null)
    {
        ...
        if ($res = Db::getInstance()->executeS($dq)) {
            ...
            foreach ($res as $item) {
                $sql = '
                    SELECT cn.`date_add` AS last_visit, cn.ip_address, c.*, arl.title, arl.content, arl.discount_name
                    ...
                    LEFT JOIN `' . _DB_PREFIX_ . 'ets_abancart_campaign_country` acc ON(acc.id_ets_abancart_campaign=' . (int)$item['id_ets_abancart_campaign'] . ' AND IF(a.id_customer is NOT NULL, acc.id_country=a.id_country, acc.id_country=' . (int)$context->country->id . '))
                    ...
                    WHERE  c.id_customer > 0
                        ...
                        AND IF(' . (int)$item['is_all_lang'] . ' != 1, cl.id_ets_abancart_campaign is NOT NULL AND cl.id_lang = c.id_lang, 1)
                        AND IF(' . (int)$item['is_all_country'] . ' != 1, (a.id_country > 0 AND a.id_country=acc.id_country) OR acc.id_country = -1 OR (a.id_customer is NULL AND acc.id_country=' . (int)$context->country->id . '), 1) 
                ...
                if ($customers) {
                    foreach ($customers as $customer) {
                        ...
                        $queueUid = md5((int)$item['id_ets_abancart_campaign'] . '-' . trim($customer['email']) . '-' . (int)$customer['id_customer']);
                        if (isset($customerQueues[$queueUid])) {
                            if ($customerQueues[$queueUid] < $item['time']) {
                                ...
                                if (isset($trackingSQLs[$queueUid]) && $trackingSQLs[$queueUid]) {
                                    foreach ($trackingSQLs[$queueUid] as &$trackingSQL) {
                                        foreach ($trackingSQL as &$queries) {
                                            if (isset($queries[0]))
                                                $queries[0] = preg_replace('/(0)(\s*,\s*(\"|\')(?:[0-9-\s:]+?)(\"|\')\s*,\s*(\"|\')(?:[0-9-\s:]+?)(\"|\')\s*\))$/', '1$2', $queries[0]);
                                            if (isset($queries[1]))
                                                unset($queries[1]);
                                        }
                                    }
                        ...
                        EtsAbancartTools::clearQueue([
                            'id_customer' => (int)$customer['id_customer'],
                            'email' => trim($customer['email']),
                            'id_ets_abancart_campaign' => (int)$item['id_ets_abancart_campaign'],
                            'id_ets_abancart_reminder' => (int)$item['id_ets_abancart_reminder']
                        ], false);
                        ...
                        if (isset($customer['ip_address']) && trim($customer['ip_address']) !== '')
                            $ip_address = long2ip((int)$customer['ip_address']);

                        $queueSQLs[$queueUid] = 'INSERT INTO `' . _DB_PREFIX_ . 'ets_abancart_email_queue` VALUES (NULL
                            , ' . (int)$context->shop->id . '
                            , ' . (int)$context->language->id . '
                            , NULL
                            , ' . ((int)$customer['id_customer'] > 0 ? (int)$customer['id_customer'] : 'NULL') . '
                            , ' . (int)$item['id_ets_abancart_reminder'] . '
                            , ' . (int)$item['id_ets_abancart_campaign'] . '
                            , \'' . pSQL($customer['firstname'] . ' ' . $customer['lastname']) . '\'
                            , \'' . pSQL($customer['email']) . '\'
                            , \'' . pSQL($customer['title']) . '\'
                            , \'' . pSQL($content, true) . '\'
                            , 0
                        ...
                        $trackingSQLs[$queueUid][(int)$item['id_ets_abancart_reminder']][] = 'INSERT INTO `' . _DB_PREFIX_ . 'ets_abancart_tracking`(id_ets_abancart_tracking, id_cart, id_customer, email, ip_address, id_ets_abancart_reminder, id_shop, display_times, total_execute_times, delivered, `read`, `deleted`, date_add, date_upd, customer_last_visit) VALUES (NULL
                            , NULL
                            , ' . ((int)$customer['id_customer'] > 0 ? (int)$customer['id_customer'] : 'NULL') . '
                            , \'' . pSQL($customer['email']) . '\'
                            , \'' . pSQL($ip_address) . '\'
                            ...
                            , \'' . pSQL(date('Y-m-d H:i:s')) . '\'
                            , \'' . pSQL(date('Y-m-d H:i:s')) . '\'
                            , \'' . pSQL($customer['last_visit']) . '\'
                        )';
                        if ((int)$tpl_vars['cart_rule']->id > 0) {
                            $trackingSQLs[$queueUid][(int)$item['id_ets_abancart_reminder']][] = 'INSERT INTO `' . _DB_PREFIX_ . 'ets_abancart_discount`(`id_ets_abancart_tracking`, `id_cart_rule`, `use_same_cart`, `fixed_voucher`) VALUES (LAST_INSERT_ID(), ' . (int)$tpl_vars['cart_rule']->id . ', ' . (int)$item['allow_multi_discount'] . ', ' . ($item['discount_option'] == 'fixed' ? 1 : 0) . ')';
                            $voucherCodes[$queueUid] = (int)$tpl_vars['cart_rule']->id;
                        }
                        $customerQueues[$queueUid] = $item['time'];
                    }
                }
            }
            if ($queueSQLs) {
                foreach ($queueSQLs as $query) {
                    if ($query)
