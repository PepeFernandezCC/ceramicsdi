...
class moduleDao
{
    ...
    public static function getProductCombination($id_shop, $id_product, $has_exclusion)
    {
        ...
        if (empty(\Configuration::get('PS_MULTISHOP_FEATURE_ACTIVE'))) {
            ...
            if (!empty($shareStock)) {
                $query->where('sa.id_shop_group = ' . (int) $id_shop);
            } else {
                $query->where('sa.id_shop = ' . (int) $id_shop);
            }
        }
    ...
    public static function getShopCategories($id_shop, $id_lang, $home_cat_id = null)
    {
        ...
        if ($home_cat_id !== null) {
            $translations = is_string(\GMerchantCenterPro::$conf['GMCP_HOME_CAT']) ? moduleTools::handleGetConfigurationData(\GMerchantCenterPro::$conf['GMCP_HOME_CAT'], ['allowed_classes' => false]) : \GMerchantCenterPro::$conf['GMCP_HOME_CAT'];
        }
    ...
    public static function searchProducts($search_string, $export_combination = false, $exclusion_ids = '')
    {
        ...
        $query->from('product', 'p');
        // $query->where('p.id_shop_default = ' . (int)\GMerchantCenterPro::$iShopId);
