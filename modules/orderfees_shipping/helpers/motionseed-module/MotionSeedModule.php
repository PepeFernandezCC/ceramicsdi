...
class MotionSeedModule extends Module
{
    ...
    public function registerAutoLoad()
    {
        ...
        spl_autoload_register(function ($class) use ($paths) {
            ...
            if (strtolower($parts[0]) == $this->name) {
                array_shift($parts);
            }
    ...
    public function registerStyleSheet($file, $is_front = false)
    {
        if (Tools::version_compare('1.7', _PS_VERSION_) && $is_front) {
            $this->context->controller->registerStylesheet(
                sprintf('modules-%s-%s', $this->name, basename($file, '.css')),
                '/modules/' . $this->name . '/views/css/' . $file,
                array('media' => 'all', 'priority' => is_numeric($is_front) ? $is_front : 5000)
            );
        } else {
            $this->context->controller->addCSS($this->getPathUri() . 'views/css/' . $file);
        }
    }
    
    public function registerJavascript($file, $is_front = false)
    {
        if (Tools::version_compare('1.7', _PS_VERSION_) && $is_front) {
            $this->context->controller->registerJavascript(
                sprintf('modules-%s-%s', $this->name, basename($file, '.js')),
                'modules/' . $this->name . '/views/js/' . $file,
                array('position' => 'bottom', 'priority' => is_numeric($is_front) ? $is_front : 5000)
            );
        } else {
            $this->context->controller->addJS($this->getPathUri() . 'views/js/' . $file);
        }
    }
