        ...
        foreach ($facetedSearchFilters as $facetArray) {
            ...
            switch ($facetArray['type']) {
                ...
                case 'id_feature':
                    ...
                    $facet->setType($type)->setProperty('filter_show_limit', $facetArray['filter_show_limit']);
                    $values = array();
                    foreach ($facetArray['values'] as $id => $filterArray) {
                        ...
                        if($facetArray['type'] == 'id_feature'){
                            $filter->setDisplayed(!$filterArray['hide']);
                        }
                        $facet->addFilter($filter);
                        if($facetArray['filter_type']==3)
