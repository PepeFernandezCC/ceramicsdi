...
class GcInvoiceWTaxManager implements TaxManagerInterface
{
    public static function isAvailableForThisAddress(Address $my_address)
    {
        ...
        $customer_groups = Customer::getGroupsStatic($my_id_customer); //añadir grupos del cliente
        ...
        //cambiar el if para que compare también con los grupos a los que pertenece el cliente
        $hasTaxExclGroup = count(array_intersect($customer_groups, $my_tax_excl_groups)) > 0;
        if ($hasTaxExclGroup || in_array($my_customer_id_group, $my_tax_excl_groups)) { 
    ...
    public function getTaxCalculator()
    {
        ...
        return new TaxCalculator([$my_tax]);
    }
}
