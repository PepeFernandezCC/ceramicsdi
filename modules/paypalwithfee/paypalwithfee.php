...
class Paypalwithfee extends PaymentModule
{
    ...
    public function installDB()
    {
        ...
        $return &= Db::getInstance()->execute('CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'ppwf_hash` ('
    ...
    protected function uninstallDB()
    {
        ...
        $sql[] = 'DROP TABLE IF EXISTS ' . _DB_PREFIX_ . 'ppwf_hash';
    ...
    public function hookPaymentOptions($params)
    {
        ...
        if ($ppal_fee_limit >= 0 && !$blockme) {
            ...
            if ($total_compare <= $ppal_fee_limit || $ppal_fee_limit == 0) {
                ...
                $externalOption->setCallToActionText($finalFee['fee_with_tax'] > 0 ? $this->l('Pay with Paypal') . ' +
                ' . Tools::displayPrice($finalFee['fee_with_tax']) : $this->l('Pay with Paypal'))
                ...
                if ($active && ($finalFee['total_order_no_fee'] + $finalFee['fee_with_tax'] >= 30)) {
                    ...
                    $externalOptionPaymentLater->setCallToActionText($finalFee['fee_with_tax'] > 0 ? $this->l('Pay later with Paypal') . ' +
                ' . Tools::displayPrice($finalFee['fee_with_tax']) : $this->l('Pay later with Paypal'))
                        ->setAction($path_controller)
                        ->setInputs([
                            'cancelURL' => [
                                'name' => 'cancelURL',
    ...
    )
    {
        ...
        if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false) {
            ...
            $cart_total_paid = $this->getOrderTotal($this->context->cart);
            ...
            foreach ($package_list as $id_address => $packageByAddress) {
                foreach ($packageByAddress as $id_package => $package) {
                    ...
                    if ($order_status->logable && number_format($cart_total_paid, _PS_PRICE_COMPUTE_PRECISION_)
                        ...
                        $difference = abs(number_format($cart_total_paid, _PS_PRICE_COMPUTE_PRECISION_) - number_format($amount_paid, _PS_PRICE_COMPUTE_PRECISION_));

                        // If difference exists, it is a payment error
                        if (!empty($difference)) {
    ...
    /**
     * Generates and stores a unique cart hash in the user's cookie and database.
     *
     * If no cart is provided, the current cart from the context is used.
     * The generated hash is stored as `ppwf_cart_hash` in the cookie and database.
     *
    ...
    public function updateCartHash($cart = null)
    {
        ...
        $sql = 'INSERT INTO `' . _DB_PREFIX_ . 'ppwf_hash` (`id_cart`, `hash_cart`, `date_add`, `date_upd`) 
                VALUES (' . (int)$cart->id . ', "' . pSQL($cart_hash) . '", NOW(), NOW())
                ON DUPLICATE KEY UPDATE `hash_cart` = "' . pSQL($cart_hash) . '", `date_upd` = NOW()';
        Db::getInstance()->execute($sql);
    }

    /**
     * Retrieves the cart hash from the user's cookie and database.
     *
    ...
    public function getCartHash()
    {
        ...
        $hash_from_cookie = (string)Context::getContext()->cookie->ppwf_cart_hash ?? '';
        
        if (!empty($hash_from_cookie)) {
            return $hash_from_cookie;
        }
        
        // If cookie is empty, try to get from database
        $cart = Context::getContext()->cart;
        if (!empty($cart->id)) {
            $sql = 'SELECT `hash_cart` FROM `' . _DB_PREFIX_ . 'ppwf_hash` 
    ...
    public function getOrderTotal($cart){
        ...
        $array_discount = $cart->getCartRules(CartRule::FILTER_ACTION_ALL_NOCAP);

        if (count($array_discount) > 0) {
            foreach ($array_discount as $value) {
                $discounts[]['total'] = Tools::ps_round($value['value_real'], $this->decimals);
                $discounts[]['total_no_tax'] = Tools::ps_round($value['value_tax_exc'], $this->decimals);
            }
        }


        //apply amount of products always 2 decimals.
        $product_list = $cart->getProducts(true);

        $products_total_amount = $round_active ? $cart->getOrderTotal(
            true,
            Cart::ONLY_PRODUCTS
        ) : $this->getTotalAmount($product_list);
        $products_total_amount_no_tax = $round_active ? $cart->getOrderTotal(
            false,
            Cart::ONLY_PRODUCTS
        ...
        $shipping_amount = Tools::ps_round($cart->getTotalShippingCost(), $this->decimals);
        ...
        return $round_active ? $total_amount_final : $total_amount_final_no_tax;
    }
    ...
    public function getTotalDiscounts($discounts, $tax = true)
    {
        ...
        if ($tax) {
            ...
            if (count($discounts) > 0) {
                foreach ($discounts as $discount) {
                    if(!empty($discount['total_no_tax'])){
