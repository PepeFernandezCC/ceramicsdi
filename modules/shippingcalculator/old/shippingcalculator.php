...
class ShippingCalculator extends Module
{
    ...
    public function saveShippingDaysByProvince($province_code, $province_name, $delivery_days, $delivery_days_min = null, $delivery_days_max = null)
    {
        ...
        // Validar que province_code no esté vacío y tenga al menos 2 caracteres
        // Permite códigos ISO de país (2-3 letras) para países sin provincias
        if (empty($province_code) || strlen($province_code) < 2) {
            return false;
        }

        // Limitar longitud según la estructura de la tabla (province_code varchar(10))
        $province_code = substr($province_code, 0, 10);
        $province_name = substr($province_name, 0, 255);
        ...
        $sql = 'INSERT INTO `' . _DB_PREFIX_ . 'shipping_calculator_delays`
                ...
                VALUES ("' . pSQL($province_code, true) . '", "' . pSQL($province_name, true) . '", ' . (int)$delivery_days . ', ' . $days_min_sql . ', ' . $days_max_sql . ')
                ON DUPLICATE KEY UPDATE 
                    province_name = "' . pSQL($province_name, true) . '",
