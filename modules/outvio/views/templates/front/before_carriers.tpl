...
<input type="hidden" id="outvio_gmak_val" value="{$google_maps_api_key|escape:'htmlall':'UTF-8'}" />
<input type="hidden" id="domain_url_shop" value="{$url_shop|escape:'htmlall':'UTF-8'}" />
<input type="hidden" id="city" value="{$delivery_address['city']|escape:'htmlall':'UTF-8'}" />
<input type="hidden" id="zip" value="{$delivery_address['postCode']|escape:'htmlall':'UTF-8'}" />
<input type="hidden" id="cc" value="{$delivery_address['countryCode']|escape:'htmlall':'UTF-8'}" />
...
<script>
	var outvio_use_map = '{$outvio_use_map}' === '1' || '{$outvio_use_map}' === '';
	function initMap() {
...
{if $outvio_use_map eq '1'}
    ...
    <script async defer
        src="https://maps.googleapis.com/maps/api/js?key={$google_maps_api_key|escape:'htmlall':'UTF-8'}&libraries=marker&callback=initMap">
    </script>
{/if}
