...
class exclusionTools
{
    ...
    public static function extractTmpRulesData($aData, $bNeedUpdate)
    {
        ...
        if (!empty($bNeedUpdate)) {
            exclusionDao::addTmpDataRules((int) \Context::getContext()->shop->id, $aData['sTypeValue'], moduleTools::handleSetConfigurationData($aOutputData));
        }
    ...
    {
        $sData = '';
        $sRulesName = moduleConfiguration::getRulesTypeLabel();
        ...
        if (is_array($aData)) {
            ...
            switch ($sType) {
                ...
                case 'word': // use case - rules based on word
                    ...
                    $aOutputData = [
                        'filter_1' => moduleConfiguration::getRulesWordType(),
    ...
    public static function getProductFromRules()
    {
        ...
        foreach ($aRules as $sKey => $aRule) {
            ...
            if ($aRule['type'] == 'supplier') {
                $aProductIds = exclusionDao::getProductFromSuppliers($aFilterValues['filter_2'], (int) \Context::getContext()->shop->id);
                ...
                if (!empty($aProductIds)) {
                    foreach ($aProductIds as $aProductId) {
                        ...
                        if (empty(\GMerchantCenterPro::$conf['GMCP_P_COMBOS'])) {
                            ...
                            $oProduct = new \Product($aProductId['id_product'], true, \GMerchantCenterPro::$iCurrentLang);
                            ...
                            if (!empty($aAttributes)) {
                                ...
                                $aProductIdsToExclude[] = [
                                    'id_product' => $aProductId['id_product'],
                                    'id_product_attribute' => isset($aProductId['id_product_attribute']) ? $aProductId['id_product_attribute'] : 0,
                                ];
                            }
                        }
                    }
