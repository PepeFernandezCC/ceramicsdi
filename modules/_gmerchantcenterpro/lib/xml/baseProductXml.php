...
abstract class baseProductXml
{
        ...
        public const PRICE_MAP_GERMANY = [
        30    => "105.36 EUR",
        50    => "109.79 EUR",
        70    => "115.48 EUR",
        100   => "122.69 EUR",
    ...
    /**
     * @var Currency,Shipping,Zone,Carrier
     */
    protected $data;
    ...
    public function buildProductXml()
    {
        ...
        // Check if product weight exceeds maximum allowed weight based on export mode
        // Export mode 1: Check product base weight
        // Export mode 0: Check combination weight if exists
        if (\GMerchantCenterPro::$conf['GMCP_EXPORT_MODE'] == 1) {
            // check if the product weight is greater than the max weight
            if (\GMerchantCenterPro::$conf['GMCP_MAX_WEIGHT'] > 0 && ($this->data->p->weight > \GMerchantCenterPro::$conf['GMCP_MAX_WEIGHT'])) {
                return false;
            }
        } else {
            if (!empty($this->data->c['weight'])) {
                if (\GMerchantCenterPro::$conf['GMCP_MAX_WEIGHT'] > 0 && ($this->data->c['weight'] > \GMerchantCenterPro::$conf['GMCP_MAX_WEIGHT'])) {
                    return false;
                }
            }
        }
            ...
            if (!empty($sName)) {
                ...
                if ($bExport) {
                    ...
                    if ($this->buildDetailProductXml()) {
                        ...
                        $this->data->step->image_link = moduleTools::getProductImage($this->data->p, !empty(\GMerchantCenterPro::$conf['GMCP_IMG_SIZE']) ? \GMerchantCenterPro::$conf['GMCP_IMG_SIZE'] : null, $this->data->step->images['image']);
                        ...
                        if (!empty(\GMerchantCenterPro::$conf['GMCP_ADD_IMAGES'])) {
                            ...
                            if (!empty($this->data->step->images['others']) && is_array($this->data->step->images['others'])) {
                                ...
                                foreach ($this->data->step->images['others'] as $aImage) {
                                    $sExtraImgLink = moduleTools::getProductImage($this->data->p, !empty(\GMerchantCenterPro::$conf['GMCP_IMG_SIZE']) ? \GMerchantCenterPro::$conf['GMCP_IMG_SIZE'] : null, $aImage);
                        ...
                        $taxonomy_code = Feeds::getFeedTaxonomy($this->aParams['sLangIso'], $this->aParams['sCountryIso'], $this->aParams['sCurrencyIso'], (int) (int)\Context::getContext()->shop->id);
                        ...
                        if (!empty(\GMerchantCenterPro::$conf['GMCP_CL_USE_DEFAULT_CAT'])) {
                            ...
                            $this->data->step->google_tags = customLabelDao::getTagsForXmlNoDefaultCat($this->data->p->id, $this->data->p->id_manufacturer, $this->data->p->id_supplier, (int) $this->aParams['iLangId']);
                        }

                        // get features by category
                        $this->data->step->features = featureCategoryTag::getFeaturesByCategory($this->data->p->id_category_default, (int)\Context::getContext()->shop->id);
                        ...
                        if (
                            ...
                            $this->data->step->certification = $this->getFeaturesOptions($this->data->p->id, $this->data->step->features['energy'], (int) $this->aParams['iLangId']);

                            $this->data->step->certification_name = $this->getFeaturesOptions($this->data->p->id, $this->data->step->features['energy_min'], (int) $this->aParams['iLangId']);

                            $this->data->step->certification_code = $this->getFeaturesOptions($this->data->p->id, $this->data->step->features['energy_max'], (int) $this->aParams['iLangId']);
                        }
    ...
    public function buildXmlTags()
    {
        ...
        if (
            ...
            if (\GMerchantCenterPro::$conf['GMCP_INC_STOCK'] == 2) {
                if (empty($this->data->step->availabilty_date)) {
                    if ($this->data->step->quantity > 0) {
                        ...
                        if (empty($this->data->step->availabilty_date)) {
                            if (!empty(\GMerchantCenterPro::$conf['GMCP_HANDLE_BACK_ORDER'])) {
            ...
            $minimal_price = $this->data->step->price;
            $price_no_discount = $this->data->step->price_no_discount;
            ...
            if (!empty($this->data->multipack)) {
                $price_no_discount = $this->calculatePricebymultipack($price_no_discount, $this->data->multipack);
                $minimal_price = $this->calculatePricebymultipack($minimal_price, $this->data->multipack);
            }
            ...
            */
            ...
            if ($this->data->step->price_raw < $this->data->step->price_raw_no_discount) {
                $sContent .= "\t\t" . '<g:price>' . $price_no_discount . '</g:price>' . "\n"
                    . "\t\t" . '<g:sale_price>' . $minimal_price . '</g:sale_price>' . "\n";
                if (
                ...
                $sContent .= "\t\t" . '<g:price>' . $minimal_price . '</g:price>' . "\n";
            }
            ...
            */
            ...
            if (
                ...
                && !empty(\GMerchantCenterPro::$conf['GMCP_INC_COLOR'])
            ) {
            ...
            if (
                ...
                && !empty(\GMerchantCenterPro::$conf['GMCP_INC_SIZE'])
            ) {
            ...
            // ****** TAG ENERGY ******
            if (
                !empty($this->data->step->certification)
                && !empty($this->data->step->certification_name)
            ) {
                $sContent .= "\t\t" . '<g:certification>' . "\n";
                $sContent .= "\t\t\t" . '<g:certification_authority><![CDATA[' . $this->data->step->certification . ']]></g:certification_authority>' . "\n";
                $sContent .= "\t\t\t" . '<g:certification_name><![CDATA[' . $this->data->step->certification_name . ']]></g:certification_name>' . "\n";
                if (!empty($this->data->step->certification_code)) {
                    $sContent .= "\t\t\t" . '<g:certification_code><![CDATA[' . $this->data->step->certification_code . ']]></g:certification_code>' . "\n";
                }
                $sContent .= "\t\t" . '</g:certification>' . "\n";
            ...
            if ($this->aParams['sCountryIso'] == 'FR') {
                $m2Caja = $this->getM2CajaOrFalse($this->data->step->id);
                
                if ($m2Caja != false) {
                    if (!empty($this->data->multipack)) {
                        $m2Caja = $m2Caja * $this->data->multipack;
                    }

                    $m2Caja =  number_format($m2Caja, 2, '.', '') . ' sqm';

                    $sContent .= "\t\t" . '<g:unit_pricing_measure>'. $m2Caja .'</g:unit_pricing_measure>' . "\n";
                    $sContent .= "\t\t" . '<g:unit_pricing_base_measure>1sqm</g:unit_pricing_base_measure>' . "\n";
                }
                
                /*
                if ($taxonomy_nomenclature == 'PIECE') {
                    $sContent .= "\t\t" . '<g:unit_pricing_measure>2sqm</g:unit_pricing_measure>' . "\n";
                    $sContent .= "\t\t" . '<g:unit_pricing_base_measure>1sqm</g:unit_pricing_base_measure>' . "\n";
                }
                if ($taxonomy_nomenclature == 'OTHER') {
                    $sContent .= "\t\t" . '<g:unit_pricing_measure>2sqm</g:unit_pricing_measure>' . "\n";
                    $sContent .= "\t\t" . '<g:unit_pricing_base_measure>1sqm</g:unit_pricing_base_measure>' . "\n";
                }
                */
            ...
            if ($this->aParams['sCountryIso'] == 'FR') {
                ...
                if (!empty($this->data->step->weight) && !empty($sWeightUnit)) {
                    if (in_array(\Tools::strtolower($sWeightUnit), moduleConfiguration::GMCP_WEIGHT_UNIT)) {
                        $multipack_weight = $this->data->step->weight;
                        if (!empty($this->data->multipack)) {
                            $multipack_weight = $this->data->step->weight * $this->data->multipack;
                        }
                        $sContent .= "\t\t" . '<g:shipping_weight>' . number_format($multipack_weight, 2, '.', '') . ' ' . \Tools::strtolower($sWeightUnit) . '</g:shipping_weight>' . "\n";
                    } else {
            ...
            if ($this->aParams['sCountryIso'] == 'FR') {
                if (!empty(\GMerchantCenterPro::$conf['GMCP_SHIPPING_USE'])) {
                    if (empty($this->data->step->free_shipping)) {
                        $real_shipping_cost = $this->data->step->shipping_fees;
                        if (!empty($this->data->step->weight) && !empty($sWeightUnit)) {
                            if (!empty($this->data->multipack)) {
                                $real_shipping_cost = $this->calculateParisShippingCostByWeight($this->data->step->weight, $this->data->multipack);;
                            }else{
                                $real_shipping_cost = $this->calculateParisShippingCostByWeight($this->data->step->weight, 1);
                            }
                        }

                        $sContent .= "\t\t" . '<g:shipping>' . "\n"
                            . "\t\t\t" . '<g:country>' . $this->aParams['sCountryIso'] . '</g:country>' . "\n"
                            . "\t\t\t" . '<g:price>' . $real_shipping_cost . '</g:price>' . "\n"
                            . "\t\t" . '</g:shipping>' . "\n";
                    } else {
                        $sContent .= "\t\t" . '<g:shipping>' . "\n"
                            . "\t\t\t" . '<g:country>' . $this->aParams['sCountryIso'] . '</g:country>' . "\n"
            ...
            if (!empty(\GMerchantCenterPro::$conf['GMCP_TAG_PAUSE_VALUE']) && !empty(\GMerchantCenterPro::$conf['GMCP_PAUSED_PROD'])) {
                if (is_string(\GMerchantCenterPro::$conf['GMCP_PAUSED_PROD'])) {
                    $pausedProduct = moduleTools::handleGetConfigurationData(\GMerchantCenterPro::$conf['GMCP_PAUSED_PROD']);
            ...
            /** Promotion ID **/
            $aPromotionIds = cartRulesDao::getAssocCartRules($this->data->p->id);
    ...
    public function getProductPath($iProdCatId, $iLangId)
    {
        if (is_string(\GMerchantCenterPro::$conf['GMCP_HOME_CAT'])) {
            \GMerchantCenterPro::$conf['GMCP_HOME_CAT'] = moduleTools::handleGetConfigurationData(\GMerchantCenterPro::$conf['GMCP_HOME_CAT']);
        }
        ...
        if (
            ...
            $sPath = stripslashes(\GMerchantCenterPro::$conf['GMCP_HOME_CAT'][$iLangId]);
        } else {
    ...
    public function calculatePricebymultipack($price, $multipack) {
       
        // Extraer solo el número antes de operar
        preg_match('/([\d\.]+)/', $price, $match_minimal);
            
        $price = isset($match_minimal[1]) ? (float)$match_minimal[1] : 0;
                
        $price = ( $price /1.21 ) * 1.2 * (int)$multipack;
                
        // Agregar nuevamente la moneda
        return number_format($price, 2, '.', '') . ' EUR';
           
    }

    public function calculateParisShippingCostByWeight($weight, $multipack){
        $weight *= $multipack; // peso = peso * cantidad_minima
        $priceMap = [ //mapa de precios
            30    => "95.98 EUR",       // >= de 30kg
            50    => "100.18 EUR",     // >= de 70
            70    => "105.14 EUR",     // ...
            100    => "111.37 EUR",     
            150    => "134.76 EUR",      
        ...
        // formula para pesos menores de 13kg
        if ($weight <= 13) {
            $shipping_cost = (($weight + 1) * 1.25) + 13; 
            return number_format($shipping_cost, 2, '.', '') . ' EUR';
        }
    
        //devuelve el coste de envío según el peso
        foreach ($priceMap as $maxWeight => $price) {
            if ($weight <= $maxWeight) {
                return $price;
            }
        }
    ...
    public function getM2CajaOrFalse($idProduct) {
        $product = new Product($idProduct);
        return $product->getM2CajaOrFalse($idProduct);
    }
