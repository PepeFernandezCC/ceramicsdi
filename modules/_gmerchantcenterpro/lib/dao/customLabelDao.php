...
class customLabelDao
{
    ...
    {
        if (\GMerchantCenterPro::$conf['GMCP_CL_MODE_OR_AND'] == 0) {
            $global = new \DbQuery();
            ...
            $dynamicBestSales = new \DbQuery();
            $dynamicBestSales->select('distinct(gt.id_tag),gt.name, gt.type, gt.position, gt.custom_label_set_postion');
            $dynamicBestSales->from('gmcp_tags', 'gt');
            $dynamicBestSales->leftJoin('gmcp_tags_dynamic_best_sale', 'gtdbs', 'gt.id_tag = gtdbs.id_tag');
            $dynamicBestSales->where('gtdbs.id_product = ' . 20);
            $dynamicBestSales->where('gt.active = 1');
            ...
            $dynamicBestSalesData = \Db::getInstance()->ExecuteS($dynamicBestSales);
            ...
            $database_tag_values = array_merge($globalData, $productTagData, $dynCategoriesTagData, $dynamicBestSalesData, $dynamicNewProductData, $manualCategoriesTagData, $dynamicPriceRangeTagData, $manualBrandTagData, $dynamicLastOrderedTagData, $dynamicPromotionTagData, $manualSupplierTagData, $dynamicLastNotOrderedTagData);
            ...
            // Instead of one large query, we'll use smaller specific queries for each tag type
            $tags = [];

            // 1. Tags linked to product features
            $featureQuery = new \DbQuery();
            $featureQuery->select('DISTINCT(gt.id_tag), fvl.value as name, gt.type, gt.position, gt.custom_label_set_postion');
            $featureQuery->from('gmcp_tags', 'gt');
            $featureQuery->innerJoin('gmcp_tags_dynamic_features', 'gtdf', 'gt.id_tag = gtdf.id_tag');
            $featureQuery->innerJoin('feature_lang', 'fl', 'gtdf.id_feature = fl.id_feature');
            $featureQuery->innerJoin('feature_product', 'fp', 'fl.id_feature = fp.id_feature AND fp.id_product = ' . (int) $id_product);
            $featureQuery->innerJoin('feature_value_lang', 'fvl', 'fp.id_feature_value = fvl.id_feature_value AND fvl.id_lang = ' . (int) $id_lang);
            $featureQuery->where('fl.id_lang = ' . (int) $id_lang);
            $featureQuery->where('gt.active = 1');
            $featureTags = \Db::getInstance()->executeS($featureQuery);
            $tags = array_merge($tags, $featureTags);

            // 2. Tags directly linked to product
            $productQuery = new \DbQuery();
            $productQuery->select('DISTINCT(gt.id_tag), gt.name, gt.type, gt.position, gt.custom_label_set_postion');
            $productQuery->from('gmcp_tags', 'gt');
            $productQuery->innerJoin('gmcp_tags_products', 'gtp', 'gt.id_tag = gtp.id_tag AND gtp.id_product = ' . (int) $id_product);
            $productQuery->where('gt.active = 1');
            $productTags = \Db::getInstance()->executeS($productQuery);
            $tags = array_merge($tags, $productTags);

            // 3. Tags specific to both category AND brand (AND logic)
            if ($id_category > 0 && $id_manufacturer > 0) {
                $categoryBrandQuery = new \DbQuery();
                $categoryBrandQuery->select('DISTINCT(gt.id_tag), gt.name, gt.type, gt.position, gt.custom_label_set_postion');
                $categoryBrandQuery->from('gmcp_tags', 'gt');
                $categoryBrandQuery->innerJoin('gmcp_tags_cats', 'gtc', 'gt.id_tag = gtc.id_tag AND gtc.id_category = ' . (int) $id_category);
                $categoryBrandQuery->innerJoin('gmcp_tags_brands', 'gtb', 'gt.id_tag = gtb.id_tag AND gtb.id_brand = ' . (int) $id_manufacturer);
                $categoryBrandQuery->where('gt.active = 1');
                $categoryBrandTags = \Db::getInstance()->executeS($categoryBrandQuery);
                $tags = array_merge($tags, $categoryBrandTags);
            }

            // 4. Tags specific to category only (modified query)
            if ($id_category > 0) {
                $categoryQuery = new \DbQuery();
                $categoryQuery->select('DISTINCT(gt.id_tag), gt.name, gt.type, gt.position, gt.custom_label_set_postion');
                $categoryQuery->from('gmcp_tags', 'gt');
                $categoryQuery->innerJoin('gmcp_tags_cats', 'gtc', 'gt.id_tag = gtc.id_tag AND gtc.id_category = ' . (int) $id_category);

                // Ne pas filtrer par rapport à toutes les marques, mais seulement par rapport
                // à celle du produit actuel si une correspondance existe
                if ($id_manufacturer > 0) {
                    $categoryQuery->leftJoin('gmcp_tags_brands', 'gtb', 'gt.id_tag = gtb.id_tag AND gtb.id_brand = ' . (int) $id_manufacturer);
                    $categoryQuery->where('gtb.id_tag IS NULL');
                }

                $categoryQuery->where('gt.active = 1');
                $categoryTags = \Db::getInstance()->executeS($categoryQuery);
                $tags = array_merge($tags, $categoryTags);
            }

            // 5. Tags linked to brand only (modified query)
            if ($id_manufacturer > 0) {
                $brandQuery = new \DbQuery();
                $brandQuery->select('DISTINCT(gt.id_tag), gt.name, gt.type, gt.position, gt.custom_label_set_postion');
                $brandQuery->from('gmcp_tags', 'gt');
                $brandQuery->innerJoin('gmcp_tags_brands', 'gtb', 'gt.id_tag = gtb.id_tag AND gtb.id_brand = ' . (int) $id_manufacturer);

                // Ne pas filtrer par rapport à toutes les catégories, mais seulement par rapport
                // à celle du produit actuel si une correspondance existe
                if ($id_category > 0) {
                    $brandQuery->leftJoin('gmcp_tags_cats', 'gtc', 'gt.id_tag = gtc.id_tag AND gtc.id_category = ' . (int) $id_category);
                    $brandQuery->where('gtc.id_tag IS NULL');
                }

                $brandQuery->where('gt.active = 1');
                $brandTags = \Db::getInstance()->executeS($brandQuery);
                $tags = array_merge($tags, $brandTags);
            }

            // Add other specific queries for other tag types...
            ...
            foreach ($tags as $label) {
                if ($label['custom_label_set_postion'] == 'custom_label_0') {
                    $sortedLabels[0][] = $label;
                } elseif ($label['custom_label_set_postion'] == 'custom_label_1') {
                    $sortedLabels[1][] = $label;
                } elseif ($label['custom_label_set_postion'] == 'custom_label_2') {
                    $sortedLabels[2][] = $label;
                } elseif ($label['custom_label_set_postion'] == 'custom_label_3') {
                    $sortedLabels[3][] = $label;
                } elseif ($label['custom_label_set_postion'] == 'custom_label_4') {
                    $sortedLabels[4][] = $label;
                }
            }
            ...
            // Build final result
            if (!empty($sortedLabels) && is_array($sortedLabels)) {
                foreach ($sortedLabels as $key => $rows) {
                    // Sort labels in each position by their tag position value
                    usort($rows, function ($a, $b) {
                        return (int) $a['position'] - (int) $b['position'];
                    });

                    // Take the label with the smallest position value
                    $labels['custom_label'][] = ['value' => $rows[0]['name'], 'position' => $rows[0]['custom_label_set_postion']];
                }
            }
        }
     ...
     * @return array
     */
    public static function getTagsForXmlNoDefaultCat($id_product, $id_manufacturer, $id_supplier, $id_lang)
    {
        $oProduct = new \Product((int) $id_product);
        $aProductCategories = $oProduct->getCategories();
        $in_data = implode(',', array_map('intval', $aProductCategories));
        ...
        if (\GMerchantCenterPro::$conf['GMCP_CL_MODE_OR_AND'] == 0) {
            ...
            $dynamicBestSales->from('gmcp_tags', 'gt');
            $dynamicBestSales->leftJoin('gmcp_tags_dynamic_best_sale', 'gtdbs', 'gt.id_tag = gtdbs.id_tag');
            $dynamicBestSales->where('gtdbs.id_product = ' . (int) $id_product);
            $dynamicBestSales->where('gt.active = 1');
            ...
            // 3. Tags for categories (modified for multiple categories)
            if (!empty($aProductCategories)) {
                $categoryQuery = new \DbQuery();
                $categoryQuery->select('DISTINCT(gt.id_tag), gt.name, gt.type, gt.position, gt.custom_label_set_postion');
                $categoryQuery->from('gmcp_tags', 'gt');
                $categoryQuery->innerJoin('gmcp_tags_cats', 'gtc', 'gt.id_tag = gtc.id_tag AND gtc.id_category IN (' . \pSQL($in_data) . ')');
            ...
            if ($id_manufacturer > 0) {
                ...
                if (!empty($aProductCategories)) {
                    $brandQuery->leftJoin('gmcp_tags_cats', 'gtc', 'gt.id_tag = gtc.id_tag AND gtc.id_category IN (' . \pSQL($in_data) . ')');
                    $brandQuery->where('gtc.id_tag IS NULL');
                }
