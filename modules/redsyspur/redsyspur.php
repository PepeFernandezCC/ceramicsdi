...
class Redsyspur extends PaymentModule {
	...
	public function __construct() {
		...
		$config = Configuration::getMultiple ( array (
				...
				'REDSYS_COMPROBACION_TOTAL_NOTIF',
				'REDSYS_MONEDA',
				'REDSYS_URLOK',
				'REDSYS_URLKO'
		) );
		...
		if (isset($config['REDSYS_COMPROBACION_TOTAL_NOTIF']))
			$this->REDSYS_COMPROBACION_TOTAL_NOTIF = $config['REDSYS_COMPROBACION_TOTAL_NOTIF'];
	...
	public function install() {
		if (! parent::install () 
				...
				|| ! Configuration::updateValue ( 'REDSYS_ACTIVAR_ANULACIONES', 1)
				|| ! Configuration::updateValue ( 'REDSYS_ACTIVAR_DEVOLUCIONES', 0)
				|| ! Configuration::updateValue ( 'REDSYS_COMPROBACION_TOTAL_NOTIF', 0)
	...
	public function uninstall() {
		...
		if (!Configuration::deleteByName('REDSYS_URLTPV_REDIR')
			...
			|| !Configuration::deleteByName('REDSYS_COMPROBACION_TOTAL_NOTIF')
	...
	private function _postProcess() {
		...
		if (Tools::isSubmit ( 'btnSubmit' )) {
			...
			Configuration::updateValue ( 'REDSYS_COMPROBACION_TOTAL_NOTIF', Tools::getValue ( 'REDSYS_COMPROBACION_TOTAL_NOTIF' ) );
			...
			escribirLog("DEBUG", "00000000000000000configUpdated", "REDSYS_TIPOPAGO_TARJETA:			" . Tools::getValue ( 'REDSYS_TIPOPAGO_TARJETA' ), $logLevel);
			...
			escribirLog("DEBUG", "00000000000000000configUpdated", "REDSYS_TIPOPAGO_TARJETA_INSITE: 	" . Tools::getValue ( 'REDSYS_TIPOPAGO_TARJETA_INSITE' ), $logLevel);
			escribirLog("DEBUG", "00000000000000000configUpdated", "REDSYS_MANTENER_CARRITO:     		" . Tools::getValue ( 'REDSYS_MANTENER_CARRITO' ), $logLevel);
			escribirLog("DEBUG", "00000000000000000configUpdated", "REDSYS_CORRECTOR_IMPORTE:     		" . Tools::getValue ( 'REDSYS_CORRECTOR_IMPORTE' ), $logLevel);
			...
			escribirLog("DEBUG", "00000000000000000configUpdated", "REDSYS_RESULTADO_ENMETHOD:  	    " . Tools::getValue ( 'REDSYS_RESULTADO_ENMETHOD' ), $logLevel);
			...
			escribirLog("DEBUG", "00000000000000000configUpdated", "REDSYS_ACTIVAR_ANULACIONES:         " . Tools::getValue ( 'REDSYS_ACTIVAR_ANULACIONES' ), $logLevel);
			escribirLog("DEBUG", "00000000000000000configUpdated", "REDSYS_ACTIVAR_DEVOLUCIONES:        " . Tools::getValue ( 'REDSYS_ACTIVAR_DEVOLUCIONES'), $logLevel);
			escribirLog("DEBUG", "00000000000000000configUpdated", "REDSYS_COMPROBACION_TOTAL_NOTIF:    " . Tools::getValue ( 'REDSYS_COMPROBACION_TOTAL_NOTIF' ), $logLevel);
			...
			'form' => [
				...
				'input' => [
					...
					[
                        ...
                        'hint' => 'Con esta opción activada, si el cliente realiza un pago en la pasarela pero la validación del pedido falla, se emitirá una anulación automática.',
						'values' => [
                            [
                                'id' => 'active_on',
                                'value' => 1,
...
//                        'hint' => 'Con esta opción activada, al tramitar una devolución desde Prestashop también se enviará la petición de devolución al TPV Virtual.',
					...
					[
						...
						'desc' => 'ATENCIÓN: Esta configuración modifica directamente <b>todos</b> los importes que se envían al TPV Virtual. Úsalo bajo tu propia responsabilidad y sólo si detectas problemas con todos los importes.',
						'placeholder' => 'Si quieres dividir lo que se envía entre 10, introduce 0.1; para hacerlo entre 100, 0.01; y así sucesivamente...',
						'required' => false,
					],
					[
						'type' => 'select',
                        'name' => 'REDSYS_COMPROBACION_TOTAL_NOTIF',
                        'label' => 'Realizar comprobación del importe antes de validar el pedido',
						'hint' => 'Desactívelo bajo su propia responsabilidad, y sólo si sabe lo que está haciendo o se lo ha indicado un técnico. Esta opción es útil si, por ejemplo, el módulo no está calculando correctamente el importe del transporte al validar la orden.',
						'desc' => 'Realizar una comprobación del importe que se ha recibido desde el TPV Virtual con el que tiene el carrito cargado de Prestashop. Si está activado y los importes no coinciden, no se creará ninguna orden y se ejecutará una anulación de la operación si la opción está activada.<br>Independientemente de la configuración, Prestashop podrá seguir realizando sus propias validaciones internas.',
						'options' => [
                            'query' => [
                                [
                                    'id' => '0',
...
//		$helper->currentIndex = AdminController::$currentIndex . '&' . http_build_query(['configure' => $this->name]);
		...
		$helper->fields_value['REDSYS_COMPROBACION_TOTAL_NOTIF'] = Tools::getValue('REDSYS_COMPROBACION_TOTAL_NOTIF', Configuration::get('REDSYS_COMPROBACION_TOTAL_NOTIF'));
...
//		$return .= $this->_displayRedsys();
	...
	private function createParameter($params){
		...
		$cantidad = (int) round(($cart->getOrderTotal(true, Cart::BOTH) * 100), 0);
