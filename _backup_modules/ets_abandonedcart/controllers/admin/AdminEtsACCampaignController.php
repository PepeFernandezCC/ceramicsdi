...
class AdminEtsACCampaignController extends AdminEtsACFormController
{
    ...
    public function validateIsSame($obj)
    {
        if (!$obj instanceof EtsAbancartCampaign)
            return false;
        $sql = '
                SELECT ' . pSQL($this->identifier) . ',is_all_country,is_all_lang FROM ' . _DB_PREFIX_ . pSQL($this->table) . ' c
                WHERE `enabled` = 1
                    AND `deleted` = 0
                    ...
                    ' . ($this->type !== 'customer' ? ' AND has_applied_voucher=\'' . pSQL($obj->has_applied_voucher) . '\'' : ' AND has_placed_orders=\'' . pSQL($obj->has_placed_orders) . '\' AND last_order_from' . ($obj->last_order_from ? '=\'' . pSQL($obj->last_order_from) . '\'' : ' IS NULL') . ' AND last_order_to' . ($obj->last_order_to ? '=\'' . pSQL($obj->last_order_to) . '\'' : ' IS NULL')) . ' 
                    ' . ($this->type == 'email' ? ' AND newsletter=' . (int)$obj->newsletter : '') . ' 
                    ' . ($this->type == 'customer' ? ' AND purchased_product' . ($obj->purchased_product ? '=\'' . pSQL($obj->purchased_product) . '\'' : ' IS NULL') . ' AND not_purchased_product' . ($obj->not_purchased_product ? '=\'' . pSQL($obj->not_purchased_product) . '\'' : ' IS NULL') : '') . ' 
                    ...
                    ' . (!in_array($this->type, [EtsAbancartCampaign::CAMPAIGN_TYPE_EMAIL, EtsAbancartCampaign::CAMPAIGN_TYPE_CUSTOMER]) ? ' AND has_product_in_cart = ' . (int)$obj->has_product_in_cart : '') . '
