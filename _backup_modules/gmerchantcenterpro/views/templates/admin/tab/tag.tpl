    ...
    <form class="form-horizontal" method="post" id="bt_form-advanced-tag" name="bt_form-advanced-tag">
        ...
        {if !empty($useAgegroup) && $currentTagHandle == 'agegroup'}
            <div class="card bg-light shadow-lg rounded border border-dark mb-3 mt-3 p-2">
                ...
                <div class="span alert alert-info mb-2">
                    {l s='Select how you want to assign the tag values to your products. Choose the first option to assign the same tag value to all the products in a given category. Choose the second option if the products in a category do not necessarily have the same tag value. In this case, you must first define a product feature corresponding to the tag, set the right feature value for each product to be exported and then come back here to select the corresponding feature for each category. To learn more, read our' mod='gmerchantcenterpro'}&nbsp;<a class="badge badge-info" href="https://faq.businesstech.fr/faq/202" target="_blank"><i class="icon icon-link"></i>&nbsp;{l s='FAQ about age group tags' mod='gmerchantcenterpro'}</a>
                </div>
        ...
        {if !empty($useAdult) && $currentTagHandle == 'adult'}
            <div class="card bg-light shadow-lg rounded border border-dark mb-3 mt-3 p-2">
                ...
                <div class="span alert alert-info mb-2">
                    {l s='Select how you want to assign the tag values to your products. Choose the first option to assign the same tag value to all the products in a given category. Choose the second option if the products in a category do not necessarily have the same tag value. In this case, you must first define a product feature corresponding to the tag, set the right feature value for each product to be exported and then come back here to select the corresponding feature for each category. To learn more, read our' mod='gmerchantcenterpro'}&nbsp;<a class="badge badge-info" href="https://faq.businesstech.fr/faq/222" target="_blank"><i class="icon icon-link"></i>&nbsp;{l s='FAQ about adult tags' mod='gmerchantcenterpro'}</a>
                </div>
        ...
        <div class="bulk-actions">
            ...
            <div class="card" id="bulk_action_adult">
                ...
                <p class="text-center mt-3">
                    <span class="btn btn-info btn-lg" onclick="oGmcPro.doSet('.agegroup', 'adult');">{l s='Adults' mod='gmerchantcenterpro'} </span>
                    - <span class="btn btn-info btn-lg" onclick="oGmcPro.doSet('.agegroup', 'kids');">{l s='Kids' mod='gmerchantcenterpro'}</span>
                    - <span class="btn btn-info btn-lg" onclick="oGmcPro.doSet('.agegroup', 'toddler');">{l s='Toddlers' mod='gmerchantcenterpro'}</span>
                    - <span class="btn btn-info btn-lg" onclick="oGmcPro.doSet('.agegroup', 'infant');">{l s='Infants' mod='gmerchantcenterpro'}</span>
                    - <span class="btn btn-info btn-lg" onclick="oGmcPro.doSet('.agegroup', 'newborn');">{l s='Newborns' mod='gmerchantcenterpro'}</span>
                        ...
                        <div class="value_agegroup">
                            <div class="col-xs-12">
                                <select class="agegroup" name="agegroup[{$cat.id_category|escape:'htmlall':'UTF-8'}]" id="agegroup{$cat.id_category|escape:'htmlall':'UTF-8'}">
                                    ...
                                    <option value="adult" {if $cat.agegroup=="adult"} selected{/if}>{l s='Adults' mod='gmerchantcenterpro'}</option>
                                    <option value="kids" {if $cat.agegroup=="kids"} selected{/if}>{l s='Kids' mod='gmerchantcenterpro'}</option>
                                    <option value="toddler" {if $cat.agegroup=="toddler"} selected{/if}>{l s='Toddlers' mod='gmerchantcenterpro'}</option>
                                    <option value="infant" {if $cat.agegroup=="infant"} selected{/if}>{l s='Infants' mod='gmerchantcenterpro'}</option>
                                    <option value="newborn" {if $cat.agegroup=="newborn"} selected{/if}>{l s='Newborns' mod='gmerchantcenterpro'}</option>
                                </select>
                            </div>
                        ...
                        <div class="value_gender">
                            <div class="col-xs-12">
                                <select class="gender" name="gender[{$cat.id_category|escape:'htmlall':'UTF-8'}]" id="gender{$cat.id_category|escape:'htmlall':'UTF-8'}">
                                    ...
                                    <option value="male" {if $cat.gender=="male"} selected{/if}>{l s='Men (male)' mod='gmerchantcenterpro'}</option>
                                    <option value="female" {if $cat.gender=="female"} selected{/if}>{l s='Women (female)' mod='gmerchantcenterpro'}</option>
                        ...
                        <div class="value_sizeType">
                            ...
                            <div class="col-xs-4">
                                <select class="sizeType" name="sizeType[{$cat.id_category|escape:'htmlall':'UTF-8'}]" id="sizeType{$cat.id_category|escape:'htmlall':'UTF-8'}">
                                    ...
                                    <option value="tall" {if $cat.sizeType=="tall"} selected{/if}>{l s='Tall' mod='gmerchantcenterpro'}</option>
                                    <option value="big" {if $cat.sizeType=="big"} selected{/if}>{l s='Big' mod='gmerchantcenterpro'}</option>
