...
class adminUpdate implements adminInterface
{
            ...
            if (\Tools::getIsset('bt_export')) {
                ...
                if ($bExportMode == 0) {
                    ...
                    if (!empty($aCategoryBox)) {
                        exportCategories::cleanTable(\GMerchantCenterPro::$iShopId);

                        foreach ($aCategoryBox as $iCatId) {
                            $export_category = new exportCategories();
                            $export_category->id_category = (int) $iCatId;
                            $export_category->id_shop = \GMerchantCenterPro::$iShopId;
                            $export_category->add();
                        }
                    }
                } else {
                    ...
                    if (!empty($aBrandBox)) {
                        exportBrands::cleanTable(\GMerchantCenterPro::$iShopId);

                        foreach ($aBrandBox as $iBrandId) {
                            $export_brand = new exportBrands();
                            $export_brand->id_brands = (int) $iBrandId;
                            $export_brand->id_shop = \GMerchantCenterPro::$iShopId;
                            $export_brand->add();
                        }
                    }
                }
        ...
        try {
            ...
            if (!empty($sLabelName)) {
                ...
                if (!empty($iTagId)) {
                    ...
                    $iTagId = customLabelTags::addTag(\GMerchantCenterPro::$iShopId, $sLabelName, $sLabelType, $customSetPosition, $bActivateTag, $iNextId, $sDateEnd);
                }
    ...
    private function updatePosition(array $aPost)
    {
        ...
        customLabelTags::updatePositionTag($iTagIdMoveToNewPos, $iNewPosition, \Gmerchantcenterpro::$iShopId);
        customLabelTags::updatePositionTag($iTagIdMoveToOldPos, $iOldPosition, \Gmerchantcenterpro::$iShopId);
        moduleTools::getConfiguration();
    ...
    private function updateCustomLabelDate(array $aPost = null)
    {
        ...
        $aTags = customLabelTags::getTagDate(\GMerchantCenterPro::$iShopId);
    ...
    private function updateGoogleCategoriesSync(array $aPost)
    {
        ...
        try {
            ...
            if ($sLangIso != false) {
                $sContent = moduleTools::getGoogleFile(moduleConfiguration::GMCP_GOOGLE_TAXONOMY_URL . 'taxonomy.' . $sLangIso . '.txt');
    ...
    private function updateExclusionRule(array $aPost)
    {
        ...
        try {
            ...
            if (empty($iExclusionId)) {
                if (exclusionDao::addExclusionRule($bActive, \GMerchantCenterPro::$iShopId, $sExclusionName, $sExclusionType, $sExclusionValue)) {
                ...
                if (exclusionDao::updateExclusionRule($bActive, \GMerchantCenterPro::$iShopId, $sExclusionName, $sExclusionType, $sExclusionValue, $iExclusionId)) {
                    // Use case for the product exclusion tab according to the option active
                    if (empty($bActive)) {
    ...
    private function updateNewFeed(array $aPost)
    {
        ...
        $feedExist = Feeds::feedExist($lang_code, $country_code, $currency, $taxonomy, (int) \GMerchantCenterPro::$iShopId);
        ...
        if (!empty($feedExist)) {
            ...
            $feed->id_shop = \GMerchantCenterPro::$iShopId;
