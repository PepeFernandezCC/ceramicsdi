    ...
    private function updateXmlFiles(array $aParam = null)
    {
        ...
        if (!empty($aParam['aAvailableData']) && is_array($aParam['aAvailableData'])) {
            ...
            foreach ($aParam['aAvailableData'] as $aData) {
                // check if file exist
                $sFileSuffix = moduleTools::buildFileSuffix($aData['langIso'], $aData['countryIso'], $aData['currencyIso'], \GMerchantCenterPro::$iShopId, 'product');
                $sFilePath = \GMerchantCenterPro::$sFilePrefix . '.' . $sFileSuffix . '.xml';
                $sFileSuffixReviews = moduleTools::buildFileSuffix($aData['langIso'], $aData['countryIso'], $aData['currencyIso'], \GMerchantCenterPro::$iShopId, 'reviews');
                $sFilePathReviews = \GMerchantCenterPro::$sFilePrefix . '.' . $sFileSuffixReviews . '.xml';
                $sFileSuffixLocal = moduleTools::buildFileSuffix($aData['langIso'], $aData['countryIso'], $aData['currencyIso'], \GMerchantCenterPro::$iShopId, 'local');
                $sFilePathLocal = \GMerchantCenterPro::$sFilePrefix . '.' . $sFileSuffixLocal . '.xml';
                $bLocalFileExists = true;

                if (!is_file(moduleConfiguration::GMCP_SHOP_PATH_ROOT . $sFilePath)
