...
class reviewDao
{
    ...
    {
        $sQuery = 'SELECT * from `' . _DB_PREFIX_ . 'gsr_rating` rt INNER JOIN `' . _DB_PREFIX_ . 'gsr_review` rw ON (rt.`RTG_ID` = rw.`RTG_ID`) WHERE RTG_SHOP_ID = ' . (int) \GMerchantCenterPro::$iShopId . ' AND RVW_LANG_ID=' . (int) $iLangId;
