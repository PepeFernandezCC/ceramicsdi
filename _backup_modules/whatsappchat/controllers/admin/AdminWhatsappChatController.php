...
class AdminWhatsappChatController extends ModuleAdminController
{
    ...
    {
        /* if (!$this->isBoLogged()) {
            die(json_encode(array('whatsappchat_response' => '[WhatsAppChat] Permission denied.')));
        } */
    ...
    public function printDisplayOn($value, $config)
    {
        ...
        if ($nb_selections > 5) {
            ...
            $selections[] = sprintf($this->l('and %1s more.'), $nb_selections - 5);
        }
        $tpl->assign(array(
            'entity' => $value,
    ...
    public function processSave()
    {
        ...
        foreach (Language::getIsoIds(false) as $lang) {
            $_POST['message_'.$lang['id_lang']] = rtrim(ltrim(Tools::getValue('message_'.$lang['id_lang']), '<p>'), '</p>');
        }
        */
            ...
            */
            ...
            if (version_compare(_PS_VERSION_, '1.6.1', '<')) {
                ...
                foreach (Language::getIsoIds(false) as $lang) {
                    ...
                    $whatsappchatblock->message[$id_lang] = rtrim(ltrim(Tools::getValue('message_'.$id_lang), '<p>'), '</p>');
    ...
    public function renderForm()
    {
        ...
        if (count($categories) > 0) {
            $category = new Category((int)end($categories)['id_category']);
        } else {
        ...
        if (Validate::isLoadedObject($category)) {
            ...
            $category_link = $this->context->shop->getBaseURL();
        }
