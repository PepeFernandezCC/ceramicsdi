...
<div class="product-variants js-product-variants">
    {foreach from=$groups key=id_attribute_group item=group}
        {if !empty($group.attributes) and $group.group_type != 'color'}
            <div class="clearfix product-variants-item {if $id_attribute_group === 2}product-variant-muestra{/if}">
                {* PLANATEC *}
                <div class="col-sm-3 col-xs-12">
                    {* END PLANATEC *}
                    <label>{$group.name}{l s=': ' d='Shop.Theme.Catalog'}</label>
                    {* PLANATEC
