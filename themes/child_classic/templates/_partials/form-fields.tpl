    ...
    <div class="form-group row {if !empty($field.errors)}has-error{/if} {if $field.name == 'customer_privacy'}hideThisCheck {/if}">
        <div class="col-md-12{if ($field.type === 'radio-buttons')} form-control-valign{/if}">
            {if $field.type !== 'checkbox'}
                <label class="form-control-label {$field.name}LabelClass {if $field.required} required{/if}" for="field-{$field.name}">
                    {if $field.name == 'dni'}
                        <span class="dniShowClass" style="display: inherit">{l s='dni' d='Shop.Forms.Labels'} *</span>
                        <span class="cifShowClass" style="display: none">{l s='cif' d='Shop.Forms.Labels'} *</span>
                    {else}    
                        {$field.label}
                    {/if}
                    
                    {if $field.required || $field.name == 'vat_number' || $field.name == 'company'} *{/if}
                </label><br>
            {/if}
                    ...
                    <span class="custom-checkbox ">
                        <label>
                            <input name="{$field.name}" type="checkbox" value="1"
                                    {if $field.value || $field.name == 'customer_privacy'}checked="checked"{/if} {if $field.required}required{/if}>
            ...
            {elseif $field.type === 'date'}
                ...
                {block name='form_field_item_date'}
                    ...
                    {if isset($field.availableValues.comment)}
                        <span class="form-control-comment">{$field.availableValues.comment}</span>
                    {/if}
                {/block}
            ...
            {else}
                ...
                {block name='form_field_item_other'}
                    ...
                    {* flags de errores custom *}

                    {if $field.name == 'firstname'}
                        <span id="firstname-required-error" class="custom-empty-field-error">
                            {l s='Required fields' d='Shop.Forms.Help'}
                        </span>
                    {/if}

                    {if $field.name == 'lastname'}
                        <span id="lastname-required-error" class="custom-empty-field-error">
                            {l s='Required fields' d='Shop.Forms.Help'}
                        </span>
                    {/if}
                    {if $field.name == 'company'}
                        <span id="company-required-error" class="custom-empty-field-error">
                            {l s='Required fields' d='Shop.Forms.Help'}
                        </span>
                    {/if}
                    {if $field.name == 'address1'}
                        <span id="address-required-error" class="custom-empty-field-error">
                            {l s='Required fields' d='Shop.Forms.Help'}
                        </span>
                    {/if}
                    ...
                    {if $field.name == 'vat_number'}
                        <span id="vat-required-error" class="custom-empty-field-error">
                            {l s='Required fields' d='Shop.Forms.Help'}
                        </span>
                    {/if}
