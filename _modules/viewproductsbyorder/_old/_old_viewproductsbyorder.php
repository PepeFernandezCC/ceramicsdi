...
class viewproductsbyorder extends Module
{
    ...
    private function getProductsSpeciAlarray() {
        $sql = '
            SELECT 
                p.id_product, 
            ...
            WHERE p.active = 1
            AND o.date_add BETWEEN "2024-01-01 00:00:00" AND NOW()
    ...
    private function updateOrInsertProducts($productArray) {
        foreach ($productArray as $product) {
            ...
            // Comprobar si ya existe el producto en la tabla
            $sql_check = '
                SELECT COUNT(*) 
                FROM `' . _DB_PREFIX_ . 'viewproductsbyorder`
                WHERE id_product = ' . $id_product . ' 
                AND (id_combination = ' . $id_combination . ' OR (' . $id_combination . ' IS NULL AND id_combination IS NULL))';
    
            $exists = Db::getInstance()->getValue($sql_check);
    
            if ($exists) {
                // Si existe, actualizar
                Db::getInstance()->update('viewproductsbyorder', [
                    'reference' => $reference,
                    'name' => $name,
                    'sample' => $sample,
                    'orders' => $orders,
                ...
                ], 'id_product = ' . $id_product . ' AND id_combination = ' . $id_combination);
            } else {
    ...
    public function processUpdateTable()
    {
        ...
        $productsArray = $this->getProductsSpeciAlarray();
             // Comprobar si ya existe en updatecatalog
        $this->updateOrInsertProducts($productsArray);
