...
final class DatabaseCleanerRepository extends Connection
{
    /**
     * @var string
     */
    private const CONNECTIONS_TABLE = 'connections';

    /**
     * @var string
     */
    private const CONNECTIONS_PAGE_TABLE = 'connections_page';

    /**
     * @var string
     */
    private const CONNECTIONS_SOURCE_TABLE = 'connections_source';
        ...
        try {
            ...
            foreach ($rows as $row) {
                ...
                if (\in_array($key, $config, true)) {
                    $this->getConnection()
                        ->delete($table, '`id_configuration` = ' . (int) $row['id_configuration']);
            ...
            throw new PerformanceProDatabaseException($prestaShopDatabaseException->getMessage(), $prestaShopDatabaseException->getCode(), $prestaShopDatabaseException);
        }
    }
    ...
    public function cleanConfigurationLang(): int
    {
        ...
        $this->getConnection()
            ->delete(
                $table,
                '`id_configuration` NOT IN (SELECT `id_configuration` FROM `' . _DB_PREFIX_ . 'configuration`)
        ...
        foreach ($this->bubble($this->getCheckAndFixQueries()) as $query) {
            ...
            $this->getConnection()
                ->delete(
                    bqSQL($query[0]),
                    '`' . bqSQL($query[1]) . '` NOT IN (SELECT `' . bqSQL(
                        $query[3]
                    ) . '` FROM `' . _DB_PREFIX_ . bqSQL($query[2]) . '`)'
                );
    ...
    public function cleanLangTable(): int
    {
        try {
            $tables = $this->getConnection()
                ->executeS('SHOW TABLES LIKE "' . _DB_PREFIX_ . '%_lang"');
            ...
            foreach ($tables as $table) {
                ...
                if (!$this->getConnection()->executeS(
                    'SHOW COLUMNS FROM `' . bqSQL($table) . '` LIKE "' . (int) $tableId . '"'
                )) {
                    continue;
                }
                ...
                $this->getConnection()
                    ->delete(
                        bqSQL($tableLang),
                        (int) $tableId . '` NOT IN (SELECT `' . (int) $tableId . '` FROM `' . bqSQL($table) . '`)',
                        0,
                        true,
                        false
                    );
                ...
                $this->getConnection()
                    ->delete(
                        bqSQL($tableLang),
                        '`id_lang` NOT IN (SELECT `id_lang` FROM `' . _DB_PREFIX_ . 'lang`)',
                        0,
                        true,
                        false
                    );
    ...
    public function cleanShopTable(): int
    {
        try {
            $tables = $this->getConnection()
                ->executeS('SHOW TABLES LIKE "' . _DB_PREFIX_ . '%_shop"');
            ...
            foreach ($tables as $table) {
                ...
                if (!$this->getConnection()->executeS(
                    'SHOW COLUMNS FROM `' . bqSQL($table) . '` LIKE "' . bqSQL($tableId) . '"'
                )) {
                    continue;
                }
                ...
                $this->getConnection()
                    ->delete(
                        bqSQL($tableShop),
                        '`' . bqSQL($tableId) . '` NOT IN (SELECT `' . bqSQL($tableId) . '` FROM `' . bqSQL(
                            $table
                        ) . '`)',
                        0,
                        true,
                ...
                $this->getConnection()
                    ->delete(
                        bqSQL($tableShop),
                        '`id_shop` NOT IN (SELECT `id_shop` FROM `' . _DB_PREFIX_ . 'shop`',
                        0,
                        true,
                        false
                    );
    ...
    public function cleanStockAvailable(): int
    {
        ...
        $this->getConnection()
            ->delete(
                $table,
                '`id_shop` NOT IN (SELECT `id_shop` FROM `' . _DB_PREFIX_ . 'shop`) AND `id_shop_group` NOT IN (SELECT `id_shop_group` FROM `' . _DB_PREFIX_ . 'shop_group`)'
            );
    ...
    public function cleanCartTable(int $days): int
    {
        ...
        $this->getConnection()
            ->delete(
                $table,
                '`id_cart` NOT IN (SELECT `id_cart` FROM `' . _DB_PREFIX_ . 'orders`) AND `date_add` < "' . $dateAdd . '"'
            );
    ...
    public function cleanRuleTable(int $days): int
    {
        ...
        $this->getConnection()
            ->delete(
                $table,
                '(`active` = 0 OR `quantity` = 0 OR `date_to` < ' . $dateTo . ') AND `date_add` < "' . $dateAdd . '"'
            );
    ...
    public function cleanConnectionTables(int $days): int
    {
        ...
        $this->getConnection()
            ->delete(self::CONNECTIONS_TABLE, '`date_add` < "' . $dateAdd . '"');

        $result += $this->getConnection()
            ->Affected_Rows();

        $this->optimizeTable(self::CONNECTIONS_TABLE);

        $this->getConnection()
            ->delete(
                self::CONNECTIONS_PAGE_TABLE,
                '`id_connections` NOT IN (SELECT `id_connections` FROM `' . _DB_PREFIX_ . self::CONNECTIONS_TABLE . '`)'
            );

        $result += $this->getConnection()
            ->Affected_Rows();

        $this->optimizeTable(self::CONNECTIONS_PAGE_TABLE);

        $this->getConnection()
            ->delete(
                self::CONNECTIONS_SOURCE_TABLE,
                '`id_connections` NOT IN (SELECT `id_connections` FROM `' . _DB_PREFIX_ . self::CONNECTIONS_TABLE . '`)'
            );
        ...
        $this->optimizeTable(self::CONNECTIONS_SOURCE_TABLE);
    ...
    public function cleanExpiredSpecificPrices(): int
    {
        ...
        $this->getConnection()
            ->delete($table, '`id_specific_price_rule` = 0 AND `to` > 0 AND `to` < NOW()');
    ...
    public function cleanGuestTable(): int
    {
        ...
        try {
            $this->getConnection()
                ->delete(
                    $table,
                    '(`id_customer` = 0 OR `id_customer` NOT IN (SELECT `id_customer` FROM `' . _DB_PREFIX_ . 'customer`))
            ...
            foreach ($parents as $parent) {
                ...
                foreach ($children as $child) {
                    $this->getConnection()
                        ->execute('UPDATE `' . _DB_PREFIX_ . 'tab` SET position = ' . $i++ . '
    ...
    private function daysToDate(int $date): int
    {
        ...
        if (!$result) {
            throw new PerformanceProInvalidDateException();
        }
