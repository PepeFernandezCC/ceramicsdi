...
{
    /**
     * @var adminGenerate : current object generate by _generateXml
     */
    public $oCurrentObj;
    ...
    private function generateXml($aParams = null)
    {
        try {
            ...
            if ($sType == 'product') {
                ...
                $feed = new xmlGenerateLocal($aParams);
            } elseif ($sType == 'discount') {
                $feed = new xmlGenerateDiscount($aParams);
            } elseif ($sType == 'reviews') {
                $feed = new xmlGenerateReviews($aParams);
            } else {
                return false;
            }
    ...
    private function generateFlyOutput(array $aPost = null)
    {
        ...
        try {
            ...
            $_POST['iShopId'] = !empty(\Tools::getValue('id_shop')) ? (int) \Tools::getValue('id_shop') : (int)\Context::getContext()->shop->id;
    ...
    private function generateCron(array $aPost = null)
    {
        ...
        try {
            ...
            if (empty($aContent['assign']['aErrors'])) {
                // handle the cron URL
                $sCronUrl = \Context::getContext()->link->getModuleLink(moduleConfiguration::GMCP_MODULE_SET_NAME, moduleConfiguration::GMCP_CTRL_CRON, ['id_shop' => (int)\Context::getContext()->shop->id]);
