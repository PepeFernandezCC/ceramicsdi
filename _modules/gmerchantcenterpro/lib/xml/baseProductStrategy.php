...
abstract class baseProductStrategy extends baseXml
{
    ...
    /**
     * @var baseProductStrategy : store the current obj to handle
     */
    ...
    public function loadProduct($bExportCombination = false, $bExcludedProduct = false)
    {
        ...
        return moduleDao::getProductIds($this->aParams['iShopId'], (int)$this->bExport, false, $this->aParams['iFloor'], $this->aParams['iStep'], $bExportCombination, $bExcludedProduct);
    }
    ...
    public function generate(array $aParams = null)
    {
        ...
        $iTotal = 0;
        $iProcess = 0;
        $sFeedType = \Tools::getValue('feed_type');
        ...
        try {
            ...
            if (!empty(\GMerchantCenterPro::$conf['GMCP_FREE_SHIP_PROD'])) {
                if (is_string(\GMerchantCenterPro::$conf['GMCP_FREE_SHIP_PROD'])) {
                    \GMerchantCenterPro::$conf['GMCP_FREE_SHIP_PROD'] = moduleTools::handleGetConfigurationData(\GMerchantCenterPro::$conf['GMCP_FREE_SHIP_PROD']);
                }
            ...
            Reporting::cleanTable(\Tools::strtoupper($sLangIso) . '_' . $sCountryIso . '_' . $sCurrencyIso, (int)\Context::getContext()->shop->id);
            ...
            foreach ($aProducts as $aProduct) {
                ...
                if (\Validate::isLoadedObject($oProduct) && $oProduct->active && (($oProduct->available_for_order)|| empty($oProduct->available_for_order))) {
                    ...
                    if (\GMerchantCenterPro::$bAdvancedPack && class_exists('AdvancedPack')
                    && \AdvancedPack::isValidPack($oProduct->id)) {
                        $this->getProdType('product', $aParams);
                    } else {
            ...
            if (!empty($aReporting)) {
                Reporting::addReporting(\Tools::strtoupper($sLangIso) . '_' . $sCountryIso . '_' . $sCurrencyIso, $aReporting, (int)\Context::getContext()->shop->id);
            }
        } catch (\Exception $e) {
