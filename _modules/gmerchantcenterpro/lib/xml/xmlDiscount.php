        ...
        foreach ($aDiscount as $aCurrentDiscount) {
            ...
            if ($sCurrencyDiscount == \Tools::getValue('currency_iso')) {
                ...
                if (!empty($aCurrentDiscount['gift_product'])) {
                    ...
                    $oProduct = new \Product($aCurrentDiscount['gift_product'], true, \GMerchantCenterPro::$iCurrentLang);
                    if (is_object($oProduct)) {
                        $sContent .= "\t\t" . '<g:free_gift_value>' . floatval(\Product::getPriceStatic((int) $aCurrentDiscount['gift_product'], true, null, 2)) . ' ' . $sCurrencyDiscount. '</g:free_gift_value>' . "\n";
