...
class xmlCombination extends baseProductXml
{
    ...
    public function buildDetailProductXml()
    {
        ...
        if (\GMerchantCenterPro::$conf['GMCP_FEED_PREF_ID'] == 'tag-id-basic') {
            ...
            $this->data->step->id = ModuleTools::constructFeedIdsEan($idProduct, $id_lang, 'combination', $idAttribute, $separator, $this->data->c['ean13'], $country);
        } elseif (\GMerchantCenterPro::$conf['GMCP_FEED_PREF_ID'] == 'tag-id-product-ref') {
            $this->data->step->id = ModuleTools::constructFeedIdsRef($idProduct, $id_lang, 'combination', $idAttribute, $separator, $this->data->c['reference'], $country);
        }
        ...
        if ($this->data->c['minimal_quantity'] > 1) {
            $this->data->multipack = (int)$this->data->c['minimal_quantity'];
        } else {
        ...
        if (empty(\GMerchantCenterPro::$conf['GMCP_USE_GEOLOC'])) {
            if (empty($this->data->multipack)) {
                ...
                $this->data->step->price_raw = $this->data->multipack *\Product::getPriceStatic((int) $this->data->p->id, $bUseTax, (int) $this->data->c['id_product_attribute']);
                $this->data->step->price_raw_no_discount = $this->data->multipack * \Product::getPriceStatic((int) $this->data->p->id, $bUseTax, (int) $this->data->c['id_product_attribute'], 6, null, false, false);
            }
        } else {
            $geolocalized_price = moduleTools::handleGeolocPrice(\Tools::getValue('sCountryIso'), $this->data->p, $this->data->c['id_product_attribute']);
        ...
        if (!empty(\GMerchantCenterPro::$conf['GMCP_FREE_SHIPPING_PRICE'])) {
            if ((float) \Product::getPriceStatic((int) $this->data->p->id, false, null, 6, (int) $this->data->c['id_product_attribute'], false, false) >= (float) \GMerchantCenterPro::$conf['GMCP_FREE_SHIPPING_PRICE']) {
