...
class labelTools
{
    ...
    public static function handleDynamicPriceRange($iTagId, $fPriceMin, $fPriceMax)
    {
        ...
        $aProductIds = customLabelDao::getPriceRangeProduct((string)$fPriceMin, (string)$fPriceMax);
    ...
    {
        $aOrders = \Order::getOrdersIdByDate($sLastOrderedStart, $sLastOrderedEnd);
        $productForExport = moduleDao::getProductIds((int)\Context::getContext()->shop->id);
        $productExportIds = [];
    ...
    public static function handleDynamicPromotion($iTagId, $sLastOrderedStart, $sLastOrderedEnd)
    {
        ...
        $aProducts = \Product::getPricesDrop(\GMerchantCenterPro::$iCurrentLang, 0, 100000, false, null, null);
        $aProductIds = [];
    ...
    public static function updateCustomLabelFeedProcess()
    {
        ...
        $aActiveTags = customLabelTags::getActive((int)\Context::getContext()->shop->id);
                ...
                if ($aTag['type'] == 'dynamic_price_range') {
                    if (!empty($aTag['id_tag'])) {
                        if (!empty($aTagDataSaved)) {
                            ...
                            self::cleanTag((int) $aTag['id_tag'], $aTag['type']);
                            self::handleDynamicPriceRange((int) $aTag['id_tag'], (float) $aTagDataSaved['price_min'], (float) $aTagDataSaved['price_max']);
                        }
                    }
                }
