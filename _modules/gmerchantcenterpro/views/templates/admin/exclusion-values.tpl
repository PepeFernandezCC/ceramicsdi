...
{if isset($aFormatSuppliers) && !empty($aFormatSuppliers)}
	{* suppliers tree *}
	<div id="bt_brands">
			...
			<div class="col-xs-12 col-md-5 col-lg-4">
				...
				<table cellspacing="0" cellpadding="0" class="table  table-bordered table-striped" style="width: 100%;">
					{foreach from=$aFormatSuppliers name=supplier key=iKey item=aSupplier}
						<tr class="alt_row">
							<td>
								{if isset($aSupplier.id)}{$aSupplier.id|escape:'htmlall':'UTF-8'}{/if}
							</td>
							<td>
								<input type="checkbox" name="bt_supplier-box[]" class="bt_supplier-box" id="bt_supplier-box_{if isset($aSupplier.id)}{$aSupplier.id|escape:'htmlall':'UTF-8'}{/if}" value="{if isset($aSupplier.id)}{$aSupplier.id|escape:'htmlall':'UTF-8'}{/if}" {if isset($aSupplier.ckecked) && $aSupplier.ckecked == true}checked="checked" {/if} />
							</td>
							<td>
								<i class="icon icon-folder{if isset($aSupplier.checked) && !empty($aSupplier.checked)}-open{/if}">&nbsp;&nbsp;<span style="font-size:12px;"></i><span>{if isset($aSupplier.name)}{$aSupplier.name|escape:'htmlall':'UTF-8'}{/if}</span>
							</td>
						</tr>
					{/foreach}
				</table>
...
{/if}
{*USE CASE FOR WORDS*}
{if isset($aWordExlusionTypeWord) && !empty($aWordExlusionTypeWord)}
		...
		<div class="col-xs-12 col-md-5 col-lg-4">
			<select name="bt-exclusion-word-type" id="bt-exclusion-word-type" onchange="$('#loadingGoogleRulesDiv').show();var sTypeValue = $('#bt-exclusion-type').val();var iWordType = $('#bt-exclusion-word-type').val();oGmcPro.ajax('{$sURI|escape:'htmlall':'UTF-8'}', '{$sCtrlParamName|escape:'htmlall':'UTF-8'}={$sController|escape:'htmlall':'UTF-8'}&sAction={$aQueryParams.excludeValue.action|escape:'htmlall':'UTF-8'}&sType={$aQueryParams.excludeValue.type|escape:'htmlall':'UTF-8'}&bRefresh=true&sExclusionType=' + sTypeValue +'&iWordType=' + iWordType + '&iRuleId=' {if !empty($iRuleId)} + {$iRuleId|escape:'htmlall':'UTF-8'}{else} + '0' {/if}, 'bt-word-text-value', 'bt-word-text-value', null, null, 'loadingGoogleRulesDiv')" ;>
				...
				{foreach from=$aWordExlusionTypeWord item=sWordExclusionType key=sKey}
					<option value="{if isset($sKey)}{$sKey|escape:'htmlall':'UTF-8'}{/if}">
						{if isset($sWordExclusionType)}{$sWordExclusionType|escape:'htmlall':'UTF-8'}{/if}
					</option>
				{/foreach}
			</select>
		</div>
		...
		<div class="col-xs-12 col-md-5 col-lg-4">
			<select name="bt-exclusion-feature" id="bt-exclusion-feature" onchange="$('#loadingGoogleRulesDiv').show();var iFeature = $('#bt-exclusion-feature').val();oGmcPro.ajax('{$sURI|escape:'htmlall':'UTF-8'}', '{$sCtrlParamName|escape:'htmlall':'UTF-8'}={$sController|escape:'htmlall':'UTF-8'}&sAction={$aQueryParams.excludeValue.action|escape:'htmlall':'UTF-8'}&sType={$aQueryParams.excludeValue.type|escape:'htmlall':'UTF-8'}&bRefresh=true&iFeatureId=' + iFeature + '&iRuleId=' {if !empty($iRuleId)} + {$iRuleId|escape:'htmlall':'UTF-8'}{else} + '0' {/if}, 'bt-exclusion-feature-value', 'bt-exclusion-feature-value', null, null, 'loadingGoogleRulesDiv');">
				...
				{foreach from=$aFeatures item=sFeature key=sKey}
					<option value="{if isset($sFeature.id_feature)}{$sFeature.id_feature|escape:'htmlall':'UTF-8'}{/if}"
						{if isset($aDataRule) && isset($aDataRule.exclusionOn) && isset($sFeature.id_feature) && $aDataRule.exclusionOn == $sFeature.id_feature}selected="selected"{/if}>
						{if isset($sFeature.name)}{$sFeature.name|escape:'htmlall':'UTF-8'}{/if}
					</option>
				{/foreach}
			</select>
		</div>
	...
	<script type="text/javascript">
		...
		{/literal}
			...
			oGmcPro.aParamsAutcomplete = {
				sInputSearch: '#bt_search-p',
				sExcludeNoProducts: '#bt_exclude-no-products',
				sExcludeProducts: '#bt_excluded-products',
				sHiddenProductNames: '#hiddenProductNames',
				sHiddenProductIds: '#hiddenProductIds'
			};
			// autocomplete
			oGmcPro.autocomplete('{$sURI|escape:'javascript':'UTF-8'}&sAction={$aQueryParams.searchProduct.action|escape:'htmlall':'UTF-8'}&sType={$aQueryParams.searchProduct.type|escape:'htmlall':'UTF-8'}', '#bt_search-p');
