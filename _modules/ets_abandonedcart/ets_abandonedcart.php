...
class Ets_abandonedcart extends Module
{
    ...
    public function hookDisplayHeader()
    {
        ...
        $css_files = [];
        if (($save_shopping_cart = (int)Configuration::get('ETS_ABANCART_SAVE_SHOPPING_CART'))
            && $controller !== 'order'
    ...
    public function hookActionCartSave($params)
    {
        if (isset($this->context->customer) && $this->context->customer->id > 0 //&& $this->context->customer->isLogged()
            ...
            if ($cart->getLastProduct()
                ...
                && $customer->id > 0
                && !EtsAbancartUnsubscribers::isUnsubscribe($this->context->customer->id)
            ) {
    ...
    public function addCartRule(EtsAbancartReminder $reminder, $id_customer = 0)
    {
        ...
        $cart_rule->id_customer = 0;//$id_customer;
