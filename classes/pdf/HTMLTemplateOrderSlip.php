...
class HTMLTemplateOrderSlipCore extends HTMLTemplateInvoice
{
    ...
    public function __construct(OrderSlip $order_slip, Smarty $smarty)
    {
        ...
        $idFacturaAbono = (((int) $this->order_slip->id)>=57) ? ((int) $this->order_slip->id - 56) : (int) $this->order_slip->id;
        $this->title = sprintf(HTMLTemplateOrderSlip::l('%1$s%2$06d'), $prefix, $idFacturaAbono);
    ...
    public function getContent()
    {
        ...
        // PLANATEC
        // Originalmente, no muestra el detalle de productos devueltos si tienen valor 0, pero como tenemos
        // productos (las muestras) con valor 0, lo debemos tener en cuenta
        if ($this->order_slip->amount >= 0) {
        ...
        $this->smarty->assign([
            ...
            'invoice_complete_number' => Configuration::get('PS_INVOICE_PREFIX', (int) $this->order->id_lang, null, $this->order->id_shop) . sprintf('%06d', $this->order->invoice_number) // PLANATEC
